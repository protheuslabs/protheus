...
const POLICY_REQUIRE_LEASE_SCOPES = String(
  process.env.POLICY_ROOT_REQUIRE_LEASE_SCOPES
    || 'strategy_mode_escalation,strategy_profile_mutation,strategy_profile_gc_apply,strategy_profile_risk_escalation,autonomy_self_change_apply,workflow_external_orchestration'
)
  .split(',')
  .map((s) => String(s || '').trim())
  .filter(Boolean);
