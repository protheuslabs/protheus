...
| BL-028 | todo | Outcome calibration by proposal type (post BL-027 companion) | Track pass-rate by proposal type and tune admission thresholds using per-type shipped/no-change outcomes | Weekly calibrator updates bounded threshold offsets per proposal type (`collector_remediation`, `opportunity_capture`, etc.) and writes audit receipts for every change |
