...
function classifyMetric(metricText: unknown, targetText: unknown, sourceText: unknown): string {
  ...
  if (metric === 'reply_or_interview_count') return 'reply_or_interview_count';
  if (metric === 'artifact_count' || metric === 'experiment_artifact' || metric === 'collector_success_runs' || metric === 'hypothesis_signal_lift') return 'artifact_count';
