...
const { compileProposalSuccessCriteria } = require('./success_criteria_compiler');
...
function suggestionBlob(proposal) {
...
function countMeasurableCriteria(proposal) {
...
function hasRollbackSignal(proposal, blob) {
  const p = proposal && typeof proposal === 'object' ? proposal : {};
...
function passB(proposal) {
  const risk = normalizeRisk(proposal && proposal.risk);
  const blob = suggestionBlob(proposal);
  const type = normalizeText(proposal && proposal.type).toLowerCase();
  const actionSpec = proposal && proposal.action_spec && typeof proposal.action_spec === 'object' ? proposal.action_spec : {};
  const objectiveId = normalizeText(proposal && proposal.meta && proposal.meta.directive_objective_id || proposal && proposal.meta && proposal.meta.objective_id || actionSpec.objective_id);
