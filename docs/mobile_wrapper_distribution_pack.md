...
```bash
node systems/ops/mobile_wrapper_distribution_pack.js build --owner=jay --target=android_termux --version=0.1.0
node systems/ops/mobile_wrapper_distribution_pack.js verify --owner=jay --target=android_termux --strict=1
node systems/ops/mobile_wrapper_distribution_pack.js rollback --owner=jay --target=android_termux --reason=manual_gate
```
