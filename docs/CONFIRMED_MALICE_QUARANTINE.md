...
`systems/helix/confirmed_malice_quarantine.ts` implements the permanent quarantine lane for confirmed hostile copies.
...
```bash
node systems/helix/confirmed_malice_quarantine.js status
node systems/helix/confirmed_malice_quarantine.js evaluate --input-json='{"sentinel":{"tier":"confirmed_malice","score":4}}' --apply=1
node systems/helix/confirmed_malice_quarantine.js release --human-approved=1
```
