...
```bash
node systems/observability/metrics_exporter.js run 2026-02-26 --window=daily
```
...
```bash
node systems/observability/trace_bridge.js span \
...
```bash
node systems/observability/trace_bridge.js summary --hours=24
```
...
```bash
node systems/observability/slo_alert_router.js route 2026-02-26 --min-level=warn
```
...
`systems/spine/spine.js` can emit observability lanes automatically.
