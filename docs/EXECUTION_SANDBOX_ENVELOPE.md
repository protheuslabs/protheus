...
```bash
node systems/security/execution_sandbox_envelope.js status
node systems/security/execution_sandbox_envelope.js evaluate-workflow --step-id=step_1 --step-type=command --command="node script.js"
node systems/security/execution_sandbox_envelope.js evaluate-actuation --kind=browser_automation --context='{"risk_class":"shell"}'
```
