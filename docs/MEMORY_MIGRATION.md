...
- `V3-RACE-CONF-005`: Added N-API build-surface compatibility lane (`systems/memory/napi_build_surface_compat.ts`) with canonical scripts (`build:memory`, `postinstall:memory`, `memory:napi-build:status`) and published matrix artifacts (`docs/MEMORY_BUILD_SURFACE.md`, `state/memory/napi_build_surface_compat/build_matrix.json`) while preserving daemon-first production defaults.
