...
```bash
node systems/workflow/client_relationship_manager.js case-open --client-id=client_a --channel=email --tier=standard
node systems/workflow/client_relationship_manager.js event --case-id=client_a_case --type=negotiation --handled-by=auto --workflow-id=wf_123
node systems/workflow/client_relationship_manager.js evaluate --days=30 --strict=1
