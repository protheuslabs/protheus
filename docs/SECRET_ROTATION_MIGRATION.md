...
- Rotate provider-side credentials for each active secret id in `config/secret_broker_policy.json`.
...
```bash
node systems/security/secret_rotation_migration_auditor.js attest \
  --operator-id=$USER \
