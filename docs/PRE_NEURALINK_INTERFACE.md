...
```bash
node systems/symbiosis/pre_neuralink_interface.js ingest --channel=voice --signal="plan next sprint" --consent-state=granted
node systems/symbiosis/pre_neuralink_interface.js route --apply=0
node systems/symbiosis/pre_neuralink_interface.js handoff-contract --write=1
...
- Route requests are evaluated via `systems/eye/eye_kernel.js`.
