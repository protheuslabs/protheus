...
```bash
node systems/helix/helix_admission_gate.js candidate --source=assimilation --capability-id=cap_example
node systems/helix/helix_admission_gate.js admit --candidate-json="$CANDIDATE_JSON" --apply=1 --doctor-approved=1
node systems/helix/helix_admission_gate.js status
```
