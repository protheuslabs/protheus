...
```bash
node systems/security/secure_heartbeat_endpoint.js issue-key --client-id=primary
node systems/security/secure_heartbeat_endpoint.js receive --payload-json='{"heartbeat_id":"hb_1","ts":"2026-02-27T00:00:00.000Z"}' --key-id=<key_id> --ts=<unix_sec> --signature=<hex>
...
node systems/security/secure_heartbeat_endpoint.js serve --host=127.0.0.1 --port=8787
```
...
See `config/secure_heartbeat_endpoint_policy.json`.
