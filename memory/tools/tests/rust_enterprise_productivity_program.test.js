...
const SCRIPT = path.join(ROOT, 'systems', 'ops', 'rust_enterprise_productivity_program.js');
const POLICY_PATH = path.join(ROOT, 'config', 'rust_enterprise_productivity_program_policy.json');
...
function run(args) {
  return spawnSync('node', [SCRIPT, ...args], {
    ...
    env: {
      ...process.env,
      RUST_ENTERPRISE_PRODUCTIVITY_PROGRAM_POLICY_PATH: POLICY_PATH
    }
  });
}
...
function fail(msg) {
  console.error(`❌ rust_enterprise_productivity_program.test.js: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  if (Number(list.item_count || 0) < 12) fail('expected at least 12 V5 Rust productivity items');
  ...
  console.log(`rust_enterprise_productivity_program.test.js: OK (${targetId})`);
}
