...
const DUAL = path.join(ROOT, 'systems', 'ops', 'state_kernel_dual_write.js');
...
function main() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'state-kernel-dual-write-'));
  ...
  const legacyStatePath = path.join(tmp, 'state', 'legacy', 'eye_state.json');
  ...
  r = run(CUTOVER, ['set-mode', '--mode=dual_write'], env);
  assert.strictEqual(r.status, 0, r.stderr || r.stdout || 'set-mode dual_write should pass');

  r = run(DUAL, ['mirror', '--organ-id=eye_kernel', '--payload-json={"route":"ok"}', `--fs-path=${legacyStatePath}`], env);
  assert.strictEqual(r.status, 0, r.stderr || r.stdout || 'dual write mirror should pass');
  let payload = parseJson(r.stdout);
  assert.ok(payload && payload.ok === true, 'dual mirror payload should be ok');
  assert.strictEqual(payload.wrote_fs, true, 'legacy fs should be written in dual_write mode');
  assert.strictEqual(payload.wrote_sqlite, true, 'sqlite should be written in dual_write mode');
  assert.ok(fs.existsSync(legacyStatePath), 'legacy fs path should exist');

  r = run(KERNEL, ['organ-get', '--organ-id=eye_kernel'], env);
  ...
  assert.strictEqual(payload.row.state.route, 'ok', 'sqlite state should mirror payload');

  console.log('state_kernel_dual_write.test.js: OK');
}

try {
  main();
} catch (err) {
  console.error(`state_kernel_dual_write.test.js: FAIL: ${err.message}`);
  process.exit(1);
}
