...
function candidate(id, proposalType, opts = {}) {
  return {
    ...
    steps: opts.steps || [
      { id: 'execute', type: 'command', command: 'node systems/autonomy/autonomy_controller.js run <date>', retries: 1, timeout_ms: 120000 },
      { id: 'verify', type: 'gate', command: 'node systems/autonomy/strategy_execute_guard.js run <date>', retries: 0, timeout_ms: 120000 },
...
function run() {
  ...
  const safeCandidate = candidate('wf_safe', 'external_intel', {
    ...
    steps: [
      { id: 'preflight', type: 'gate', command: 'node systems/spine/contract_check.js', retries: 0, timeout_ms: 60000 },
      { id: 'execute', type: 'command', command: 'node systems/autonomy/autonomy_controller.js run <date>', retries: 1, timeout_ms: 120000 },
      { id: 'rollback', type: 'command', command: 'node systems/autonomy/strategy_execute_guard.js rollback <date>', retries: 0, timeout_ms: 90000 },
      { id: 'verify', type: 'gate', command: 'node systems/autonomy/strategy_execute_guard.js run <date>', retries: 0, timeout_ms: 120000 },
      { id: 'receipt', type: 'receipt', command: 'state/autonomy/receipts/<date>.jsonl', retries: 0, timeout_ms: 30000 }
    ],
    shipped_rate: 0.55,
