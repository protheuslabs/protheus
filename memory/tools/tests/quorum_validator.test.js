...
try {
  let out = evaluateProposalQuorum({
    ...
    risk: 'high',
    suggested_next_command: 'node systems/security/policy_rootd.js authorize --scope=strategy.mode --dry-run',
    meta: { directive_objective_id: 'T1_goal' },
    action_spec: {
      command: 'node systems/security/policy_rootd.js authorize --scope=strategy.mode --dry-run',
  ...
  out = evaluateProposalQuorum({
    ...
    type: 'self_modification',
    risk: 'high',
    suggested_next_command: 'node systems/security/policy_rootd.js authorize --scope=strategy.mode',
    meta: {},
    action_spec: { command: 'node systems/security/policy_rootd.js authorize --scope=strategy.mode' },
    success_criteria: []
  });
  assert.strictEqual(out.requires_quorum, true, 'high-tier proposal should require quorum');
