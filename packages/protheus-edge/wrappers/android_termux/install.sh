...
node "$ROOT_DIR/client/runtime/systems/ops/mobile_wrapper_distribution_pack.ts" verify --owner="${PROTHEUS_OWNER:-operator}" --target=android_termux --policy="$ROOT_DIR/client/runtime/config/mobile_wrapper_distribution_pack_policy.json" --strict=1 --apply=0
