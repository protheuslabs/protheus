...
fn payload_json(argv: &[String]) -> Result<Value, String> {
    if let Some(raw) = lane_utils::parse_flag(argv, "payload", false) {
        return serde_json::from_str::<Value>(&raw)
            .map_err(|err| format!("dify_bridge_payload_decode_failed:{err}"));
    }
    if let Some(raw_b64) = lane_utils::parse_flag(argv, "payload-base64", false) {
        let bytes = BASE64_STANDARD
            .decode(raw_b64.as_bytes())
            .map_err(|err| format!("dify_bridge_payload_base64_decode_failed:{err}"))?;
        let text = String::from_utf8(bytes)
            .map_err(|err| format!("dify_bridge_payload_utf8_decode_failed:{err}"))?;
...
fn normalize_bridge_path(root: &Path, raw: &str) -> Result<String, String> {
    ...
    {
        return Err("dify_bridge_path_outside_allowed_surface".to_string());
    }
