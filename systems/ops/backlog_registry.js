...
function renderView(title, rows, generatedAt) {
  const body = [];
  body.push(`# ${title}`);
  body.push('');
...
function loadPolicy(policyPath = DEFAULT_POLICY_PATH) {
  ...
  return {
    ...
    active_statuses: Array.isArray(raw.active_statuses)
      ? raw.active_statuses.map((row) => normalizeToken(row, 80)).filter(Boolean)
      : base.active_statuses,
    archive_statuses: Array.isArray(raw.archive_statuses)
      ? raw.archive_statuses.map((row) => normalizeToken(row, 80)).filter(Boolean)
...
function buildArtifacts(policy) {
