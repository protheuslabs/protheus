...
function runContractViaRust(payload: AnyObj) {
...
function parsePlanFromArg(args: AnyObj) {
...
function normalizeTaskStatus(v: unknown) {
...
function normalizePlan(rawPlan: unknown) {
  const plan = rawPlan && typeof rawPlan === 'object' ? rawPlan as AnyObj : {};
...
function summarizeTasks(tasks: AnyObj[]) {
...
function hasSkippedTasks(tasks: AnyObj[]) {
  return tasks.some((task) => normalizeTaskStatus(task.status) === 'skipped');
}

function validateOrderedExecution(tasks: AnyObj[]) {
  let firstNonCompletedSeen = false;
  for (const task of tasks) {
    const status = normalizeTaskStatus(task.status);
    const isCompleted = status === 'completed';
    if (!isCompleted) firstNonCompletedSeen = true;
...
function runContract(policy: AnyObj, args: AnyObj) {
...
function main() {
  ...
  if (!policy.enabled) emit({ ok: false, error: 'rust50_sprint_contract_disabled' }, 1);
