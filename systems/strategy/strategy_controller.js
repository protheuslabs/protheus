...
const { defaultStrategyDraft, STORE_ABS_PATH, normalizeMode, normalizeExecutionMode, ensureStrategyState, readStrategyState, setStrategyState, mutateStrategyState, validateProfileInput, upsertProfile, intakeSignal, materializeFromQueue, touchProfileUsage, gcProfiles } = require('../adaptive/strategy/strategy_store');
...
function saveAuditState(state) {
    ...
    fs.writeFileSync(STRATEGY_CONTROLLER_AUDIT_STATE_PATH, JSON.stringify({
...
function saveSnapshotIndex(index) {
    ...
    fs.writeFileSync(STRATEGY_SNAPSHOT_INDEX_PATH, JSON.stringify({
        version: '1.0',
        updated_ts: nowIso(),
...
function resolveSnapshotRef(ref) {
    ...
    for (let i = index.snapshots.length - 1; i >= 0; i--) {
        ...
        if (!row || typeof row !== 'object')
            continue;
        if (String(row.id || '') === raw && row.path && fs.existsSync(String(row.path)))
...
function runPolicyRootAuthorize({ scope, target, approvalNote, leaseToken, source }) {
    ...
    if (leaseToken)
        cliArgs.push(`--lease-token=${String(leaseToken).trim()}`);
    ...
    for (const dateStr of dates) {
        ...
        for (const evt of rows) {
            ...
            if (result === 'no_candidates')
                continue;
            if (!byId[strategyId])
                byId[strategyId] = { uses_window: 0, last_used_ts: null };
...
function cmdGet(args) {
    ...
    if (queue)
        out.intake_queue = state.intake_queue.slice(0, limit);
...
function cmdQueue(args) {
    ...
    if (status !== 'all')
        rows = rows.filter((q) => String(q.status || '') === status);
...
function cmdMutateProfile(args) {
    ...
    mutateStrategyState(null, (state) => {
        ...
        if (idx < 0)
            throw new Error(`strategy_not_found:${id}`);
...
function cmdSyncUsage(args) {
    ...
    const next = mutateStrategyState(null, (state) => {
        ...
        state.profiles = state.profiles.map((profile) => {
            ...
            if (row.last_used_ts)
                usage.last_used_ts = String(row.last_used_ts);
            if (!Array.isArray(usage.use_events))
                usage.use_events = [];
            if (row.last_used_ts) {
...
function main() {
    ...
    try {
        ...
        if (cmd === 'materialize')
            return cmdMaterialize(args);
        if (cmd === 'set-profile')
            return cmdSetProfile(args);
        if (cmd === 'mutate-profile')
            return cmdMutateProfile(args);
        if (cmd === 'touch-use')
            return cmdTouchUse(args);
        if (cmd === 'sync-usage')
            return cmdSyncUsage(args);
        if (cmd === 'gc')
            return cmdGc(args);
        if (cmd === 'restore')
