...
const { DEFAULT_STATE_DIR: GLOBAL_BUDGET_STATE_DIR, DEFAULT_EVENTS_PATH: GLOBAL_BUDGET_EVENTS_PATH, DEFAULT_AUTOPAUSE_PATH: GLOBAL_BUDGET_AUTOPAUSE_PATH, loadSystemBudgetState, saveSystemBudgetState, projectSystemBudget, recordSystemBudgetUsage, writeSystemBudgetDecision, loadSystemBudgetAutopauseState, setSystemBudgetAutopause, evaluateSystemBudgetGuard } = require('../budget/system_budget');
...
function loadPolicy() {
    ...
    return {
        ...
        token_budget: {
            ...
            modules: parseTokenBudgetModules(tokenBudget.modules, Math.round(clampNumber(tokenBudget.default_daily_token_cap, 0, 100000000, 0)), Math.round(clampNumber(tokenBudget.default_per_request_token_cap, 0, 10000000, 0)))
        },
        leases: {
...
function evaluateTokenBudget(policy, state, moduleName, requestedTokens, requestedCells) {
    ...
    if (!out.enabled || requestedTokens <= 0)
        return out;
    ...
    if (moduleCap > 0) {
        ...
        if (ratio >= Number(tokenPolicy.hard_ratio || 0.95))
            out.pressure = 'hard';
        else if (ratio >= Number(tokenPolicy.soft_ratio || 0.8))
            out.pressure = 'soft';
    }
...
function pruneExpired(state) {
    ...
    for (const [name, ent] of Object.entries(state.allocations || {})) {
        const row = ent && typeof ent === 'object' ? ent : {};
...
function hardwareBounds(policy, planPayload) {
    ...
    const capByRam = Number.isFinite(ramGb) && perCellRam > 0
        ...
        : pool.max_cells;
    const globalMax = Math.max(pool.min_cells, Math.min(pool.max_cells, classCap, capByCpu, capByRam));
    return {
...
function cmdRequest(args) {
    ...
    const budgetDecision = (() => {
        if (tokenBudget.allow !== true)
            return 'deny';
        if (tokenBudget.action === 'degrade')
            return 'degrade';
        return 'allow';
    })();
