...
const { GLOBAL_BUDGET_DEFAULT_DIR, DEFAULT_EVENTS_PATH, DEFAULT_AUTOPAUSE_PATH, evaluateSystemBudgetGuard, loadSystemBudgetAutopauseState, writeSystemBudgetDecision, setSystemBudgetAutopause } = require('../budget/system_budget');
const ROUTE_EXECUTE_BUDGET_ENABLED = !['0', 'false', 'no', 'off'].includes(String(process.env.ROUTE_EXECUTE_BUDGET_ENABLED || '1').trim().toLowerCase());
...
function runRouteTask({ task, tokensEst, repeats14d, errors30d, skipHabitId, mode, forceModel = null, dryRun = false }) {
    ...
    if (skipHabitId)
        args.push('--skip_habit_id', String(skipHabitId));
    if (mode)
        args.push('--mode', String(mode));
    if (forceModel)
        args.push('--force_model', String(forceModel));
...
function collectModelCandidatesFromValue(value, outSet) {
    ...
    if (Array.isArray(value)) {
        for (const item of value)
            collectModelCandidatesFromValue(item, set);
        return set;
    }
    if (typeof value === 'object') {
        ...
        for (const child of nested)
            collectModelCandidatesFromValue(child, set);
        return set;
    }
    const raw = String(value || '').trim();
    if (raw && /[a-z0-9]/i.test(raw) && (raw.includes('/') || raw.includes(':')))
        set.add(raw);
    return set;
}
function extractObservedModels(stdout, stderr) {
    ...
    for (const row of rows)
        collectModelCandidatesFromValue(row, seen);
...
function isAutoHabitFlow(out) {
    if (!out || typeof out !== 'object')
        return false;
    if (out.auto_habit_flow === true)
        return true;
    const execSpec = out.executor;
    if (!execSpec || !Array.isArray(execSpec.args))
        return false;
...
function executorSig(out) {
    ...
    if (!ex || !ex.cmd || !Array.isArray(ex.args))
        return null;
...
function parseRouteTaskOutput(routed) {
    ...
    if (!routed.stdout)
        return { ok: false, error: 'route_task returned no stdout', out: null };
    const out = parseJson(routed.stdout);
    if (!out)
        return { ok: false, error: 'failed to parse route_task JSON output', out: null };
    return { ok: true, out };
}
function main() {
    ...
    if (!budgetBlocked && summary.mode === 'deep-thinker' && summary.deep_thinker && summary.deep_thinker.enabled === true) {
        ...
        for (const row of passes) {
            if (!row || row.ok !== true || !row.signature)
                continue;
    ...
    if (summary.model_changed) {
        process.stderr.write(`ROUTER_SWITCH: ${summary.previous_model || 'none'} -> ${summary.selected_model || 'none'} ` +
    ...
    if (autoHabitFlow) {
        ...
        if (crystallizeStdout)
            process.stdout.write(crystallizeStdout);
        if (crystallizeStderr)
            process.stderr.write(crystallizeStderr);
        ...
        const crystallizedHabitId = String((crystalSummary && crystalSummary.habit_id) || summary.suggested_habit_id || '').trim();
        if (crystallizedHabitId)
            summary.suggested_habit_id = crystallizedHabitId;
        let habitRun = null;
        if (finalExitCode === 0 && crystallizedHabitId) {
            ...
            if (runStdout)
                process.stdout.write(runStdout);
            if (runStderr)
                process.stderr.write(runStderr);
        ...
        process.stdout.write(JSON.stringify({
            ...
            execution_metrics: {
                ...
                route_model_attestation: verifyRouteModelAttestation(selectedModel, `${crystallizeStdout}\n${habitRun ? String(habitRun.stdout || '') : ''}`, `${crystallizeStderr}\n${habitRun ? String(habitRun.stderr || '') : ''}`)
            },
            auto_habit_flow: {
