...
function normalizeRefList(v, maxItems = 12, maxLen = 64) {
    ...
    for (const row of src) {
        ...
        seen.add(id);
        out.push(id);
        if (out.length >= Math.max(1, Number(maxItems) || 12))
            break;
    }
    return out;
}
...
function ensureGuardOrExit() {
    if (String(process.env.EYES_INTAKE_SKIP_GUARD || '') === '1')
        return;
...
function createEye(args) {
    ...
    if (strategyId)
        eye.strategy_id = strategyId;
    if (campaignIds.length > 0)
        eye.campaign_ids = campaignIds;
    ...
    if (!Array.isArray(registry.eyes))
        registry.eyes = [];
