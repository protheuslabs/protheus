...
function toStateContractPayload(raw, opts = {}) {
    ...
    const fallbackCap = toPositiveInt(opts.fallback_token_cap, toPositiveInt(src.token_cap, 0));
    return {
...
function effectiveStrategyBudget(opts = {}) {
    const defaults = {
        daily_token_cap: toPositiveInt(opts.daily_token_cap != null ? opts.daily_token_cap : process.env.SYSTEM_BUDGET_DEFAULT_DAILY_TOKEN_CAP, 4000),
        max_tokens_per_action: toPositiveInt(opts.max_tokens_per_action != null ? opts.max_tokens_per_action : process.env.SYSTEM_BUDGET_DEFAULT_MAX_TOKENS_PER_ACTION, 1600),
        ...
        min_projected_tokens_for_burn_check: toPositiveInt(opts.min_projected_tokens_for_burn_check != null
            ? opts.min_projected_tokens_for_burn_check
            : process.env.SYSTEM_BUDGET_DEFAULT_MIN_PROJECTED_TOKENS_FOR_BURN_CHECK, 800)
    };
...
function projectSystemBudget(state, requestTokens, opts = {}) {
    ...
    if (ratio >= hardRatio)
        pressure = 'hard';
    else if (ratio >= softRatio)
        pressure = 'soft';
    let projectedPressure = 'none';
    if (projectedRatio >= hardRatio)
        projectedPressure = 'hard';
    else if (projectedRatio >= softRatio)
        projectedPressure = 'soft';
    return {
        request_tokens_est: safeRequest,
...
function sumMonthTokens(dateStr, opts = {}) {
    ...
    for (const name of names) {
        if (!String(name).startsWith(targetMonth))
            continue;
...
function evaluateSystemBudgetGuard(input = {}, opts = {}) {
    ...
    const burnRateMultiplier = clamp(opts.burn_rate_multiplier != null ? opts.burn_rate_multiplier : process.env.SYSTEM_BUDGET_BURN_RATE_MULTIPLIER, 1, 10, 1.5);
    const minDaysForBurnBaseline = toPositiveInt(opts.min_days_for_burn_baseline != null ? opts.min_days_for_burn_baseline : process.env.SYSTEM_BUDGET_BURN_BASELINE_DAYS, 3);
    const minProjectedTokensForBurnCheck = toPositiveInt(strategy.min_projected_tokens_for_burn_check, 800);
...
function cmdGuard(args) {
    ...
    const requestTokens = toPositiveInt(args.request_tokens_est != null ? args.request_tokens_est : args.tokens_est, 0);
...
function main() {
    ...
    if (cmd === 'decision')
        return cmdDecision(args);
    if (cmd === 'autopause')
        return cmdAutopause(args);
    if (cmd === 'migrate')
