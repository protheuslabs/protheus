...
function tokenizeCode(v) {
    ...
    return Array.from(new Set(blob
        .split(/[:\s_-]+/)
...
function classifyTierFromSignal(row) {
    ...
    if (severity === 'high' || risk === 'high' || CRITICAL_CODE_RE.test(blob))
        return 1;
    if (severity === 'medium' || risk === 'medium')
        return 2;
    return 3;
}
function classifyTierFromEscalation(row) {
    ...
    if (risk === 'high')
        return 1;
    if (CRITICAL_CODE_RE.test(`${stage} ${err}`))
        return 1;
    return 2;
}
...
function makePainCandidate(row, dateStr) {
    if (!row || typeof row !== 'object')
        return null;
    if (String(row.type || '') !== 'pain_signal')
        return null;
    ...
    if (failureTier > MAX_CAPTURE_TIER)
        return null;
...
function makeEscalationCandidate(row, dateStr) {
    if (!row || typeof row !== 'object')
        return null;
    if (String(row.type || '') !== 'autonomy_human_escalation')
        return null;
    const d = toDateOnly(row.ts);
    if (!d || d !== dateStr)
        return null;
    if (!INCLUDE_RESOLVED_ESCALATIONS && String(row.status || '').toLowerCase() === 'resolved')
        return null;
...
function loadCandidates(dateStr) {
    ...
    return Array.from(merged.values())
        .sort((a, b) => {
        if (a.failure_tier !== b.failure_tier)
            return a.failure_tier - b.failure_tier;
...
function runBridge(dateStr, maxNodes) {
    ...
    const alreadyPointeredToday = new Set(existingPointerRows.map((r) => String(r && r.item_hash || '').trim()).filter(Boolean));
