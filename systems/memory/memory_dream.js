...
function enrichThemesWithOlderMemory(themes, rows, recentDates) {
    if (!Array.isArray(themes) || themes.length === 0)
        return { themes: [], older_links_total: 0 };
    ...
    for (const theme of nextThemes) {
        if (olderLinksTotal >= DEFAULT_OLDER_TOTAL)
            break;
        ...
        for (const ent of indexEntries) {
            ...
            if (usedKeys.has(key))
                continue;
            if (recentNodeIds.has(String(ent.node_id || '')))
                continue;
            const entryDate = dateFromFileRef(ent.file);
            if (oldestRecent && entryDate && entryDate >= oldestRecent)
                continue;
            let score = 0;
            if (ent.tags.includes(token))
                score += 8;
            if (String(ent.summary || '').toLowerCase().includes(token))
                score += 3;
            if (String(ent.node_id || '').toLowerCase().includes(token))
                score += 2;
            if (score <= 0)
                continue;
            candidates.push({
...
function themeRows(rows) {
    ...
    for (const row of rows) {
        ...
        for (const tok of tokens) {
            ...
            ent.rows.push(row);
            if (row && row.eye_id)
                ent.eye_ids.add(String(row.eye_id));
            if (row && row.node_id)
                ent.node_ids.add(String(row.node_id));
    ...
    for (const ent of themes.values()) {
        ...
        if (occurrenceCount < 2)
            continue;
        if (eyeDiversity < 2 && occurrenceCount < 3)
            continue;
