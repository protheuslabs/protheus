...
function sharedTags(a, b) {
    ...
    for (const t of (b.tags || [])) {
        if (!NOISE_TAGS.has(t) && aset.has(t) && !out.includes(t))
            out.push(t);
    }
    out.sort();
    return out;
...
function loadConnectionIndex() {
    ...
    if (!base.keys || typeof base.keys !== 'object')
        base.keys = {};
    return base;
}
...
function buildConnections(rows, dateStr) {
    ...
    for (let i = 0; i < rows.length; i++) {
        for (let j = i + 1; j < rows.length; j++) {
            ...
            if (shared.length < SHARED_TAG_MIN)
                continue;
...
function buildAdaptiveSuggestions(rows, connections, dateStr) {
    ...
    for (const c of connections) {
        for (const t of c.shared_tags || []) {
            if (!byTag.has(t))
                byTag.set(t, { tag: t, connections: [], uids: new Set() });
    ...
    for (const bucket of byTag.values()) {
        if (adaptiveTags.has(bucket.tag))
            continue;
        if (bucket.connections.length < SUGGESTION_MIN_CONNECTIONS)
            continue;
    ...
    suggestions.sort((a, b) => {
        if (b.relation_count !== a.relation_count)
            return b.relation_count - a.relation_count;
        return String(a.theme_tag).localeCompare(String(b.theme_tag));
    });
    if (suggestions.length === 0)
        return { added: 0, file: null };
