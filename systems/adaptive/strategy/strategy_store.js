...
const { ADAPTIVE_ROOT, readJson, ensureJson, setJson, mutateJson } = require('../core/layer_store');
...
function normalizeMode(v, fallback = 'hyper-creative') {
    ...
    if (GENERATION_MODES.has(key))
        return key;
    return fallback;
}
function normalizeExecutionMode(v, fallback = 'score_only') {
    const key = String(v || '').trim().toLowerCase();
    if (EXECUTION_MODES.has(key))
        return key;
    return fallback;
}
...
function defaultStrategyDraft(seed = {}) {
    ...
    const objectivePrimary = cleanText(seed.objective && seed.objective.primary
        ? seed.objective.primary
        : (seed.summary || seed.prompt || `Improve outcomes for ${name}`), 180);
    return {
        version: '1.0',
        id,
        name,
        ...
        risk_policy: {
            ...
            max_risk_per_action: clampNumber(seed.risk_policy && seed.risk_policy.max_risk_per_action, 0, 100, 35)
        },
        admission_policy: {
            allowed_types: [],
            blocked_types: [],
...
function computeTrustScore(item) {
    ...
    if (source.includes('outcome_fitness') || source.includes('strategy_scorecards'))
        score += 12;
    if (source.includes('cross_signal') || source.includes('sensory_trends'))
        score += 8;
    if (source === 'manual')
        score += 5;
...
function queueDropReason(item, policy, nowMs) {
    ...
    if (createdMs > 0 && (nowMs - createdMs) > (ttlHours * 60 * 60 * 1000))
        reasons.push('queue_ttl_expired');
    if (Number(src.attempts || 0) >= maxAttempts)
        reasons.push('queue_max_attempts_exceeded');
    if (Number((src.evidence_refs || []).length || 0) < minEvidence)
        reasons.push('evidence_missing');
    if (Number(src.trust_score || 0) < minTrust)
        reasons.push('trust_score_low');
    if (String(src.summary || '').trim().length < 16)
...
function normalizeProfile(raw, nowTs) {
    ...
    draft.objective.primary = cleanText(draft.objective.primary || src.objective_primary || '', 220) || `Improve outcomes for ${draft.name}`;
    ...
    const requestedExecutionMode = normalizeExecutionMode(src.execution_mode
        || (src.execution_policy && src.execution_policy.mode)
        || (draft.execution_policy && draft.execution_policy.mode), 'score_only');
    ...
    draft.generation_policy = {
        mode: normalizeMode(src.generation_mode
            || (src.generation_policy && src.generation_policy.mode)
            || (draft.generation_policy && draft.generation_policy.mode), 'hyper-creative')
    };
    const uidCandidate = cleanText(src.uid, 64);
    const uid = uidCandidate && isAlnum(uidCandidate)
        ? uidCandidate
    ...
    return {
        ...
        generated_mode: normalizeMode(src.generated_mode
            || src.generation_mode
            || (draft.generation_policy && draft.generation_policy.mode), 'hyper-creative'),
...
function validateProfileInput(rawProfile, opts = {}) {
    ...
    if (!allowElevatedMode && mode !== 'score_only')
...
function ensureStrategyState(filePath, meta = {}) {
    const abs = asStorePath(filePath);
    return normalizeState(ensureJson(abs, defaultStrategyState, {
        ...meta,
...
function setStrategyState(filePath, nextState, meta = {}) {
    ...
    return normalizeState(setJson(abs, normalized, {
        ...meta,
        source: meta.source || 'systems/adaptive/strategy/strategy_store.js',
...
function mutateStrategyState(filePath, mutator, meta = {}) {
    ...
    if (typeof mutator !== 'function')
        throw new Error('strategy_store: mutator must be function');
    return normalizeState(mutateJson(abs, (current) => {
...
function upsertProfile(filePath, profileInput, meta = {}) {
    let result = null;
    const next = mutateStrategyState(filePath, (state) => {
        const ts = nowIso();
...
function materializeFromQueue(filePath, queueUid, draftInput, meta = {}) {
    ...
    if (!qid)
        throw new Error('strategy_store: queue_uid_required');
    let result = null;
    const next = mutateStrategyState(filePath, (state) => {
        const ts = nowIso();
        const idx = state.intake_queue.findIndex((q) => String(q.uid || '') === qid);
        if (idx < 0)
            throw new Error(`strategy_store: queue_item_not_found:${qid}`);
        ...
        const upsert = validateProfileInput({
            ...
            queue_ref: qid,
            generated_mode: normalizeMode(draftObj.generated_mode
                || draftObj.generation_mode
                || queueItem.recommended_generation_mode, 'hyper-creative'),
...
function touchProfileUsage(filePath, strategyId, ts, meta = {}) {
    ...
    if (!sid)
        throw new Error('strategy_store: strategy_id_required');
    ...
    const next = mutateStrategyState(filePath, (state) => {
        ...
        if (idx < 0)
            throw new Error(`strategy_store: strategy_not_found:${sid}`);
