...
function normalizeList(v: unknown, fallback: string[] = []) {
  if (Array.isArray(v)) {
...
function runJson(scriptPath: string, args: string[], timeoutMs: number) {
...
function isInQuietHours(policy: AnyObj, nowDate: Date) {
  if (policy.quiet_hours.enabled !== true) return false;
...
function cmdRun(args: AnyObj) {
  ...
  if (!out.skipped) {
    ...
    for (let i = 0; i < maxCycles; i += 1) {
      if (Number(day.cycles || 0) >= Number(policy.budget_guard.max_cycles_per_day || 0)) break;
      const cycle: AnyObj = {
        cycle_index: i + 1,
        observe: null,
        proposals: [],
      ...
      for (const target of targets) {
        if (cycle.proposals.length >= Number(policy.proposal_cap_per_cycle || 0)) break;
        if (Number(day.proposals || 0) >= Number(policy.budget_guard.max_proposals_per_day || 0)) break;
        const propose = runJson(
          policy.scripts.loop,
          [
            'propose',
        ...
        if (proposalId) day.proposals = Number(day.proposals || 0) + 1;
      }

      for (const row of cycle.proposals) {
        if (!row || !row.proposal_id) continue;
        const allowApply = applyRequested
          && cycle.runs.filter((r: AnyObj) => r && r.apply_requested === true).length < Number(policy.apply_cap_per_cycle || 0)
          && Number(day.applies || 0) < Number(policy.budget_guard.max_applies_per_day || 0);
        const runOut = runJson(
          policy.scripts.loop,
          [
            'run',
        ...
        if (applied) day.applies = Number(day.applies || 0) + 1;
        cycle.runs.push({
      ...
      cycle.distill = runJson(
        policy.scripts.distiller,
        [
          'distill',
  ...
  out.proposals_created = Array.isArray(out.cycles)
    ? out.cycles.reduce((acc: number, cycle: AnyObj) => acc + Number(Array.isArray(cycle.proposals) ? cycle.proposals.filter((r: AnyObj) => !!(r && r.proposal_id)).length : 0), 0)
    : 0;
