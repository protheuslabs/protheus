...
function resolveVerifyStep(name) {
    ...
    if (!def || !Array.isArray(def.command) || def.command.length < 1)
        return null;
    return {
        key,
...
function rootCauseFromVerification(steps) {
    ...
    if (!failed)
        return null;
    if (failed.error)
        return `verify_step_error:${String(failed.id || 'unknown')}`;
...
function runVerificationPlan(steps, opts = {}) {
    ...
    for (const step of rows) {
        if (!step || !Array.isArray(step.command) || step.command.length < 1)
            continue;
...
function optValue(name, fallback = null) {
    ...
    if (fromEq)
        return fromEq.slice(pref.length);
    const idx = process.argv.indexOf(`--${name}`);
    if (idx >= 0 && idx + 1 < process.argv.length) {
        const nxt = process.argv[idx + 1];
        if (!String(nxt).startsWith('--'))
            return nxt;
        return '';
    }
    return fallback;
...
function runNodeJson(absScript, args) {
    ...
    if (!out)
        throw new Error(`no stdout from ${path.relative(REPO_ROOT, absScript)}`);
...
function resolveCommit(commitArg) {
    ...
    if (r.status !== 0)
        throw new Error(`git rev-parse failed: ${String(r.stderr || '').trim()}`);
...
function saveTrialsStore(store) {
    ...
    if (fs.existsSync(TRIALS_PATH))
        fs.copyFileSync(TRIALS_PATH, snapshot);
    else
        fs.writeFileSync(snapshot, JSON.stringify({ trials: [] }, null, 2) + '\n', 'utf8');
    ...
    try {
        ...
        try {
            if (fs.existsSync(snapshot))
                fs.copyFileSync(snapshot, TRIALS_PATH);
...
function evaluateMetrics(base, cur, thresholds) {
    ...
    if (attempts < Number(thresholds.min_attempts || 0))
        reasons.push(`attempts_below_min:${attempts}<${thresholds.min_attempts}`);
    if (revRateCur > Number(thresholds.max_reverted_rate || 0))
        reasons.push(`reverted_rate_high:${revRateCur.toFixed(3)}>${thresholds.max_reverted_rate}`);
...
function recoverInterruptedRevert() {
    ...
    if (!tx || tx.active !== true)
        return { recovered: false, reason: 'no_active_revert_tx' };
...
function startValidatedCmd(dateStr) {
    ...
    const note = compactText(optValue('note', ''), 240);
    const approvalNote = compactText(optValue('approval-note', note || `approve start-validated ${commit}`), 320) || `approve start-validated ${commit}`;
    const leaseToken = compactText(optValue('lease-token', process.env.CAPABILITY_LEASE_TOKEN || ''), 8192);
    const thresholds = trialThresholds();
    ...
    if (policyRoot.required && !policyRoot.ok) {
        ...
        if (!phases.apply.ok) {
            ...
            if (!verify.ok)
                rootCause = verify.root_cause || 'verify_failed';
        }
    }
...
function main() {
    ...
    if (cmd === 'start')
        return startCmd(dateStr);
    if (cmd === 'start-validated')
        return startValidatedCmd(dateStr);
    if (cmd === 'evaluate')
        return evaluateCmd(dateStr);
    if (cmd === 'status')
        return statusCmd();
    usage();
    process.exit(2);
