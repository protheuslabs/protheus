...
const { normalizeTrit, tritLabel, majorityTrit, TRIT_PAIN, TRIT_UNKNOWN, TRIT_OK } = require('../../lib/trit');
...
function normalizeWordToken(v, maxLen = 80) {
    return cleanText(v, maxLen)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
        .replace(/_+/g, '_')
...
function tokenize(v) {
    return Array.from(new Set(cleanText(v, 1200)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
        .split(' ')
        .map((row) => row.trim())
        .filter((row) => row.length >= 3))).slice(0, 64);
}
function escapeRegex(v) {
    return String(v == null ? '' : v).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
function patternToWordRegex(pattern) {
...
function normalizeList(v, maxLen = 80) {
    if (Array.isArray(v)) {
...
function stableId(seed, prefix = 'inv') {
...
function runtimePaths(policyPath) {
...
function normalizeBandMap(raw, base, lo, hi) {
...
function normalizeImpactMap(raw, base, lo, hi) {
    const src = raw && typeof raw === 'object' ? raw : {};
    return {
...
function normalizeTargetMap(raw, base, lo, hi) {
    const src = raw && typeof raw === 'object' ? raw : {};
    return {
...
function normalizeTargetPolicy(raw, base) {
    const src = raw && typeof raw === 'object' ? raw : {};
    return {
...
function loadPolicy(policyPath = DEFAULT_POLICY_PATH) {
    ...
    function normalizeOutputChannel(name) {
        ...
        return {
            ...
            require_sandbox_verification: toBool(srcOut.require_sandbox_verification, baseOut.require_sandbox_verification === true)
        };
    }
    function normalizeAxiomList(rawAxioms, baseAxioms) {
        ...
        return out
            ...
            const patterns = (Array.isArray(item.patterns) ? item.patterns : [])
                .map((x) => cleanText(x, 140).toLowerCase())
                .filter(Boolean)
                .slice(0, 20);
            ...
            const action_terms = (Array.isArray(signals.action_terms) ? signals.action_terms : [])
                .map((x) => cleanText(x, 80).toLowerCase())
                .filter(Boolean)
                .slice(0, 24);
            ...
            const object_terms = (Array.isArray(signals.object_terms) ? signals.object_terms : [])
                ...
                .filter(Boolean)
                .slice(0, 24);
            const min_signal_groups = clampInt(item.min_signal_groups, 0, 3, ((action_terms.length ? 1 : 0)
                + (subject_terms.length ? 1 : 0)
                + (object_terms.length ? 1 : 0)));
            if (!id || (!patterns.length && !regex.length && !intent_tags.length))
                return null;
            return {
                id,
                patterns,
    ...
    function normalizeHarnessSuite(rawSuite, baseSuite) {
    ...
    return {
        ...
        runtime: {
            ...
            test: {
                allow_constitution_inversion: toBool(runtimeTestRaw.allow_constitution_inversion, base.runtime.test.allow_constitution_inversion)
            }
        },
        maturity: {
            target_test_count: clampInt(maturityRaw.target_test_count, 1, 10000, base.maturity.target_test_count),
            score_weights: {
                pass_rate: clampNumber(scoreWeightsRaw.pass_rate, 0, 1, base.maturity.score_weights.pass_rate),
                non_destructive_rate: clampNumber(scoreWeightsRaw.non_destructive_rate, 0, 1, base.maturity.score_weights.non_destructive_rate),
            ...
            max_target_rank_by_band: normalizeBandMap(maturityRaw.max_target_rank_by_band, base.maturity.max_target_rank_by_band, 1, 10)
        },
        impact: {
            max_target_rank: normalizeImpactMap(impactRaw.max_target_rank, base.impact.max_target_rank, 1, 10)
        },
        certainty_gate: {
            thresholds: {
            ...
            allow_zero_for_legendary_critical: toBool(certaintyRaw.allow_zero_for_legendary_critical, base.certainty_gate.allow_zero_for_legendary_critical)
        },
        targets: {
        ...
        tier_transition: {
            ...
            human_veto_min_target_rank: clampInt(tierTransitionRaw.human_veto_min_target_rank, 1, 10, base.tier_transition.human_veto_min_target_rank),
            use_success_counts_for_first_n: toBool(tierTransitionRaw.use_success_counts_for_first_n, base.tier_transition.use_success_counts_for_first_n),
            safe_abort_relief: toBool(tierTransitionRaw.safe_abort_relief, base.tier_transition.safe_abort_relief),
            first_live_uses_require_human_veto: normalizeTargetMap(tierTransitionRaw.first_live_uses_require_human_veto, base.tier_transition.first_live_uses_require_human_veto, 0, 100000),
            minimum_first_live_uses_require_human_veto: normalizeTargetMap(tierTransitionRaw.minimum_first_live_uses_require_human_veto, base.tier_transition.minimum_first_live_uses_require_human_veto, 0, 100000),
            window_days_by_target: normalizeTargetMap(tierTransitionRaw.window_days_by_target, base.tier_transition.window_days_by_target, 1, 3650),
            minimum_window_days_by_target: normalizeTargetMap(tierTransitionRaw.minimum_window_days_by_target, base.tier_transition.minimum_window_days_by_target, 1, 3650)
        },
        shadow_pass_gate: {
            enabled: toBool(shadowPassRaw.enabled, base.shadow_pass_gate.enabled),
            require_for_live_apply: toBool(shadowPassRaw.require_for_live_apply, base.shadow_pass_gate.require_for_live_apply),
            required_passes_by_target: normalizeTargetMap(shadowPassRaw.required_passes_by_target, base.shadow_pass_gate.required_passes_by_target, 0, 100000),
            max_critical_failures_by_target: normalizeTargetMap(shadowPassRaw.max_critical_failures_by_target, base.shadow_pass_gate.max_critical_failures_by_target, 0, 100000),
            window_days_by_target: normalizeTargetMap(shadowPassRaw.window_days_by_target, base.shadow_pass_gate.window_days_by_target, 1, 3650)
        },
        immutable_axioms: {
        ...
        creative_preference: {
            ...
            preferred_creative_lane_ids: normalizeList(creativeRaw.preferred_creative_lane_ids || base.creative_preference.preferred_creative_lane_ids, 120),
            non_creative_certainty_penalty: clampNumber(creativeRaw.non_creative_certainty_penalty, 0, 0.5, base.creative_preference.non_creative_certainty_penalty)
        },
        guardrails: {
            default_session_ttl_minutes: clampInt(guardrailsRaw.default_session_ttl_minutes, 5, 7 * 24 * 60, base.guardrails.default_session_ttl_minutes),
            max_active_sessions: clampInt(guardrailsRaw.max_active_sessions, 1, 500, base.guardrails.max_active_sessions),
            objective_id_required_min_target_rank: clampInt(guardrailsRaw.objective_id_required_min_target_rank, 1, 10, base.guardrails.objective_id_required_min_target_rank),
            max_similar_failures_by_band: normalizeBandMap(guardrailsRaw.max_similar_failures_by_band, base.guardrails.max_similar_failures_by_band, 0, 100)
        },
        library: {
            max_entries: clampInt(libraryRaw.max_entries, 100, 100000, base.library.max_entries),
            min_similarity_for_reuse: clampNumber(libraryRaw.min_similarity_for_reuse, 0, 1, base.library.min_similarity_for_reuse),
            ...
            failed_repetition_similarity_block: clampNumber(libraryRaw.failed_repetition_similarity_block, 0, 1, base.library.failed_repetition_similarity_block)
        },
        first_principles: {
            enabled: toBool(firstPrinciplesRaw.enabled, base.first_principles.enabled),
            auto_extract_on_success: toBool(firstPrinciplesRaw.auto_extract_on_success, base.first_principles.auto_extract_on_success),
            max_strategy_bonus: clampNumber(firstPrinciplesRaw.max_strategy_bonus, 0, 1, base.first_principles.max_strategy_bonus),
            allow_failure_cluster_extraction: toBool(firstPrinciplesRaw.allow_failure_cluster_extraction, base.first_principles.allow_failure_cluster_extraction),
            failure_cluster_min: clampInt(firstPrinciplesRaw.failure_cluster_min, 2, 50, base.first_principles.failure_cluster_min),
            anti_downgrade: {
                enabled: toBool(antiDowngradeRaw.enabled, base.first_principles.anti_downgrade.enabled),
                require_same_or_higher_maturity: toBool(antiDowngradeRaw.require_same_or_higher_maturity, base.first_principles.anti_downgrade.require_same_or_higher_maturity),
                prevent_lower_confidence_same_band: toBool(antiDowngradeRaw.prevent_lower_confidence_same_band, base.first_principles.anti_downgrade.prevent_lower_confidence_same_band),
                same_band_confidence_floor_ratio: clampNumber(antiDowngradeRaw.same_band_confidence_floor_ratio, 0.1, 1, base.first_principles.anti_downgrade.same_band_confidence_floor_ratio)
            }
        },
        maturity_harness: {
            enabled: toBool(harnessRaw.enabled, base.maturity_harness.enabled),
            auto_trigger_on_run: toBool(harnessRaw.auto_trigger_on_run, base.maturity_harness.auto_trigger_on_run),
            trigger_interval_hours: clampInt(harnessRaw.trigger_interval_hours, 1, 24 * 30, base.maturity_harness.trigger_interval_hours),
            max_tests_per_cycle: clampInt(harnessRaw.max_tests_per_cycle, 1, 50, base.maturity_harness.max_tests_per_cycle),
            destructive_tokens: normalizeList(harnessRaw.destructive_tokens || base.maturity_harness.destructive_tokens, 120),
            runtime_probes: {
                enabled: toBool(harnessRaw.runtime_probes && harnessRaw.runtime_probes.enabled, base.maturity_harness.runtime_probes.enabled),
                required: toBool(harnessRaw.runtime_probes && harnessRaw.runtime_probes.required, base.maturity_harness.runtime_probes.required),
                timeout_ms: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.timeout_ms, 1000, 5 * 60 * 1000, base.maturity_harness.runtime_probes.timeout_ms),
                run_red_team: toBool(harnessRaw.runtime_probes && harnessRaw.runtime_probes.run_red_team, base.maturity_harness.runtime_probes.run_red_team),
                red_team_max_cases: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.red_team_max_cases, 1, 32, base.maturity_harness.runtime_probes.red_team_max_cases),
                min_red_team_executed_cases: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.min_red_team_executed_cases, 0, 64, base.maturity_harness.runtime_probes.min_red_team_executed_cases),
                max_red_team_critical_failures: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.max_red_team_critical_failures, 0, 64, base.maturity_harness.runtime_probes.max_red_team_critical_failures),
                run_workflow_nursery: toBool(harnessRaw.runtime_probes && harnessRaw.runtime_probes.run_workflow_nursery, base.maturity_harness.runtime_probes.run_workflow_nursery),
                workflow_nursery_intent: cleanText(harnessRaw.runtime_probes && harnessRaw.runtime_probes.workflow_nursery_intent, 220) || base.maturity_harness.runtime_probes.workflow_nursery_intent,
                workflow_nursery_days: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.workflow_nursery_days, 1, 30, base.maturity_harness.runtime_probes.workflow_nursery_days),
                workflow_nursery_max_candidates: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.workflow_nursery_max_candidates, 1, 24, base.maturity_harness.runtime_probes.workflow_nursery_max_candidates),
                min_workflow_nursery_candidates: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.min_workflow_nursery_candidates, 0, 64, base.maturity_harness.runtime_probes.min_workflow_nursery_candidates),
                min_workflow_nursery_scorecards: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.min_workflow_nursery_scorecards, 0, 256, base.maturity_harness.runtime_probes.min_workflow_nursery_scorecards),
                min_workflow_adversarial_probes: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.min_workflow_adversarial_probes, 0, 1024, base.maturity_harness.runtime_probes.min_workflow_adversarial_probes),
                max_nursery_red_team_critical_fail_cases: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.max_nursery_red_team_critical_fail_cases, 0, 64, base.maturity_harness.runtime_probes.max_nursery_red_team_critical_fail_cases),
                max_nursery_adversarial_critical_failures: clampInt(harnessRaw.runtime_probes && harnessRaw.runtime_probes.max_nursery_adversarial_critical_failures, 0, 64, base.maturity_harness.runtime_probes.max_nursery_adversarial_critical_failures),
                max_nursery_regression_risk: clampNumber(harnessRaw.runtime_probes && harnessRaw.runtime_probes.max_nursery_regression_risk, 0, 1, base.maturity_harness.runtime_probes.max_nursery_regression_risk),
                require_workflow_output_snapshot: toBool(harnessRaw.runtime_probes && harnessRaw.runtime_probes.require_workflow_output_snapshot, base.maturity_harness.runtime_probes.require_workflow_output_snapshot)
            },
            test_suite: normalizeHarnessSuite(harnessRaw.test_suite || base.maturity_harness.test_suite, base.maturity_harness.test_suite)
        },
        attractor: {
            enabled: toBool(attractorRaw.enabled, base.attractor.enabled),
            min_alignment_by_target: normalizeTargetMap(attractorRaw.min_alignment_by_target, base.attractor.min_alignment_by_target, 0, 1),
            weights: {
                objective_specificity: clampNumber(attractorRaw.weights && attractorRaw.weights.objective_specificity, 0, 1, base.attractor.weights.objective_specificity),
                evidence_backing: clampNumber(attractorRaw.weights && attractorRaw.weights.evidence_backing, 0, 1, base.attractor.weights.evidence_backing),
                constraint_evidence: clampNumber(attractorRaw.weights && attractorRaw.weights.constraint_evidence, 0, 1, base.attractor.weights.constraint_evidence),
                measurable_outcome: clampNumber(attractorRaw.weights && attractorRaw.weights.measurable_outcome, 0, 1, base.attractor.weights.measurable_outcome),
                external_grounding: clampNumber(attractorRaw.weights && attractorRaw.weights.external_grounding, 0, 1, base.attractor.weights.external_grounding),
                certainty: clampNumber(attractorRaw.weights && attractorRaw.weights.certainty, 0, 1, base.attractor.weights.certainty),
                trit_alignment: clampNumber(attractorRaw.weights && attractorRaw.weights.trit_alignment, 0, 1, base.attractor.weights.trit_alignment),
                impact_alignment: clampNumber(attractorRaw.weights && attractorRaw.weights.impact_alignment, 0, 1, base.attractor.weights.impact_alignment),
                verbosity_penalty: clampNumber(attractorRaw.weights && attractorRaw.weights.verbosity_penalty, 0, 1, base.attractor.weights.verbosity_penalty)
            },
            verbosity: {
                soft_word_cap: clampInt(attractorRaw.verbosity && attractorRaw.verbosity.soft_word_cap, 8, 1000, base.attractor.verbosity.soft_word_cap),
                hard_word_cap: clampInt(attractorRaw.verbosity && attractorRaw.verbosity.hard_word_cap, 16, 2000, base.attractor.verbosity.hard_word_cap),
                low_diversity_floor: clampNumber(attractorRaw.verbosity && attractorRaw.verbosity.low_diversity_floor, 0.05, 0.95, base.attractor.verbosity.low_diversity_floor)
            }
        },
        output_interfaces: {
...
function buildOutputInterfaces(policy, mode, basePayload, opts = {}) {
...
function normalizeIsoEvents(src, maxRows = 10000) {
...
function expandLegacyCountToEvents(count, ts) {
    const n = clampInt(count, 0, 4096, 0);
    if (n <= 0)
        return [];
    return Array.from({ length: n }, () => ts);
}
function normalizeTierEventMap(src, fallback, legacyCounts = {}, legacyTs = nowIso()) {
    const out = {};
...
function defaultTierScope(legacy = {}, legacyTs = nowIso()) {
...
function normalizeTierScope(scope, legacy = {}, legacyTs = nowIso()) {
...
function cloneTierScope(scope) {
    return normalizeTierScope(scope || {});
}
function pruneTierScopeEvents(scope, retentionDays) {
    ...
    for (const metric of ['live_apply_attempts', 'live_apply_successes', 'live_apply_safe_aborts', 'shadow_passes', 'shadow_critical_failures']) {
        ...
        for (const target of TIER_TARGETS) {
            ...
            map[target] = rows.filter((row) => parseTsMs(row) >= keepCutoff).slice(-10000);
        }
        out[metric] = map;
    }
    return out;
}
function getTierScope(state, policyVersion) {
...
function loadTierGovernanceState(paths, policyVersion = '1.0') {
    ...
    for (const [version, scope] of Object.entries(scopesSrc)) {
        out.scopes[String(version)] = normalizeTierScope(scope);
    }
...
function saveTierGovernanceState(paths, state, policyVersion = '1.0', retentionDays = 365) {
    const safeVersion = cleanText(policyVersion || '1.0', 24) || '1.0';
    const src = state && typeof state === 'object' ? state : {};
    const scopesSrc = src.scopes && typeof src.scopes === 'object' ? src.scopes : {};
    const scopes = {};
    for (const [version, scope] of Object.entries(scopesSrc)) {
        scopes[String(version)] = pruneTierScopeEvents(scope, retentionDays);
    }
...
function addTierEvent(paths, policy, metric, target, ts = nowIso()) {
    ...
    if (metric === 'live_apply_attempts')
        pushTierEvent(scope.live_apply_attempts, target, ts);
    if (metric === 'live_apply_successes')
        pushTierEvent(scope.live_apply_successes, target, ts);
    if (metric === 'live_apply_safe_aborts')
        pushTierEvent(scope.live_apply_safe_aborts, target, ts);
    if (metric === 'shadow_passes')
        pushTierEvent(scope.shadow_passes, target, ts);
    if (metric === 'shadow_critical_failures')
        pushTierEvent(scope.shadow_critical_failures, target, ts);
    state.scopes[policyVersion] = scope;
    return saveTierGovernanceState(paths, state, policyVersion, tierRetentionDays(policy));
}
function countTierEvents(scope, metric, target, windowDays) {
    ...
    for (const row of rows) {
        if (parseTsMs(row) >= cutoff)
            count += 1;
    }
    return count;
}
function windowDaysForTarget(windowMap, target, fallback) {
    return clampInt(windowMap && windowMap[normalizeTarget(target || 'tactical')], 1, 3650, fallback);
}
function effectiveWindowDaysForTarget(windowMap, minimumWindowMap, target, fallback) {
...
function effectiveFirstNHumanVetoUses(tierTransition, target) {
    const key = normalizeTarget(target || 'tactical');
    const configured = clampInt(tierTransition.first_live_uses_require_human_veto && tierTransition.first_live_uses_require_human_veto[key], 0, 100000, 0);
    const minimum = clampInt(tierTransition.minimum_first_live_uses_require_human_veto
        && tierTransition.minimum_first_live_uses_require_human_veto[key], 0, 100000, 0);
    return Math.max(configured, minimum);
}
function incrementLiveApplyAttempt(paths, policy, target) {
    return addTierEvent(paths, policy, 'live_apply_attempts', target, nowIso());
}
function incrementLiveApplySuccess(paths, policy, target) {
    return addTierEvent(paths, policy, 'live_apply_successes', target, nowIso());
}
function incrementLiveApplySafeAbort(paths, policy, target) {
    return addTierEvent(paths, policy, 'live_apply_safe_aborts', target, nowIso());
}
function updateShadowTrialCounters(paths, policy, session, result, destructive) {
...
function loadHarnessState(paths) {
...
function saveHarnessState(paths, state) {
    const out = {
        schema_id: 'inversion_maturity_harness_state',
        schema_version: '1.0',
        updated_at: nowIso(),
...
function loadFirstPrincipleLockState(paths) {
...
function saveFirstPrincipleLockState(paths, state) {
    const out = {
        schema_id: 'inversion_first_principle_lock_state',
        schema_version: '1.0',
        updated_at: nowIso(),
...
function principleKeyForSession(session) {
...
function checkFirstPrincipleDowngrade(paths, policy, session, confidence) {
    const anti = policy.first_principles && policy.first_principles.anti_downgrade
        ? policy.first_principles.anti_downgrade
        : {};
    if (anti.enabled !== true)
        return { allowed: true, reason: null, key: principleKeyForSession(session), lockState: null };
    ...
    if (anti.prevent_lower_confidence_same_band === true
...
function upsertFirstPrincipleLock(paths, session, principle) {
    const lockState = loadFirstPrincipleLockState(paths);
    const key = principleKeyForSession(session);
    const existing = lockState.locks && typeof lockState.locks === 'object' ? lockState.locks[key] : null;
    ...
    const mergedConfidence = existing && typeof existing === 'object'
        ...
        : confidence;
    if (!lockState.locks || typeof lockState.locks !== 'object')
        lockState.locks = {};
    lockState.locks[key] = {
        key,
...
function detectImmutableAxiomViolation(policy, decisionInput) {
    const axiomsPolicy = policy.immutable_axioms || {};
    if (axiomsPolicy.enabled !== true)
        return [];
    ...
    const haystack = [
        ...
        ...(Array.isArray(decisionInput.filters) ? decisionInput.filters.map((x) => cleanText(x, 120)) : [])
    ].join(' ').toLowerCase();
    ...
    for (const axiom of rows) {
        ...
        const patternRegexes = patterns
            .map((pattern) => patternToWordRegex(pattern))
            .filter(Boolean);
        const regexRules = Array.isArray(axiom && axiom.regex)
            ? axiom.regex.map((row) => cleanText(row, 220)).filter(Boolean)
            : [];
        ...
        if (!id || (!patternRegexes.length && !regexHits.length && !tagRules.length))
            continue;
        const patternMatched = patternRegexes.some((re) => re.test(haystack));
        const regexMatched = regexHits.some((re) => re.test(haystack));
        const tagMatched = tagRules.some((tag) => intentTags.includes(tag));
        ...
        if (tagMatched || strictRegexMatch || structuredPatternMatch)
            hits.push(id);
    }
    return Array.from(new Set(hits));
}
function computeAttractorScore(policy, input) {
    ...
    const evidenceBacking = clampNumber((constraintHits * 0.2)
        + (measurableHits * 0.2)
        + (externalHits * 0.15)
        + (comparisonHits * 0.1)
        + (Math.min(5, evidenceCount) * 0.35), 0, 1, 0);
    const specificity = Number(clampNumber((constraintEvidence * 0.4) + (measurableEvidence * 0.35) + (externalGrounding * 0.25), 0, 1, 0).toFixed(6));
    ...
    const lowDiversityPenalty = lexicalDiversity < lowDiversityFloor
        ...
        : 0;
    const weakEvidencePenalty = 1 - clampNumber((constraintEvidence * 0.4) + (measurableEvidence * 0.3) + (externalGrounding * 0.2) + (evidenceBacking * 0.1), 0, 1, 0);
    const verbosityPenalty = Number(clampNumber((verbosityOver * weakEvidencePenalty * 0.75) + (lowDiversityPenalty * 0.25), 0, 1, 0).toFixed(6));
    ...
    const constraintWeight = Number(weights.constraint_evidence != null
        ? weights.constraint_evidence
        : (objectiveSpecificityWeight * 0.4));
    const measurableWeight = Number(weights.measurable_outcome != null
        ? weights.measurable_outcome
        : (objectiveSpecificityWeight * 0.35));
    const externalWeight = Number(weights.external_grounding != null
        ? weights.external_grounding
        : (objectiveSpecificityWeight * 0.25));
    const positiveWeightTotal = Math.max(0.0001, objectiveSpecificityWeight
    ...
    const positiveScore = ((specificity * objectiveSpecificityWeight)
        ...
        + (impactFactor * Number(weights.impact_alignment || 0))) / positiveWeightTotal;
    const score = clampNumber(positiveScore - (verbosityPenalty * verbosityPenaltyWeight), 0, 1, 0);
...
function computeMaturityScore(state, policy) {
    ...
    const weightTotal = Math.max(0.0001, Number(weights.pass_rate || 0) + Number(weights.non_destructive_rate || 0) + Number(weights.experience || 0));
    const score = ((passRate * Number(weights.pass_rate || 0))
        + (nonDestructiveRate * Number(weights.non_destructive_rate || 0))
        + (experience * Number(weights.experience || 0))) / weightTotal;
    const s = clampNumber(score, 0, 1, 0);
    const bands = policy.maturity.bands || {};
    let band = 'legendary';
    if (s < Number(bands.novice || 0.25))
        band = 'novice';
    else if (s < Number(bands.developing || 0.45))
        band = 'developing';
    else if (s < Number(bands.mature || 0.65))
        band = 'mature';
    else if (s < Number(bands.seasoned || 0.82))
        band = 'seasoned';
    return {
...
function loadMaturityState(paths, policy) {
...
function saveMaturityState(paths, policy, state) {
...
function normalizeTarget(v) {
    ...
    if (['tactical', 'belief', 'identity', 'directive', 'constitution'].includes(raw))
        return raw;
    return 'tactical';
}
function normalizeResult(v) {
    const raw = normalizeToken(v || '', 24);
    if (raw === 'success' || raw === 'neutral' || raw === 'fail' || raw === 'destructive')
        return raw;
    return '';
}
function isValidObjectiveId(v) {
    const raw = cleanText(v || '', 140);
    if (!raw)
        return false;
    if (raw.length < 6 || raw.length > 140)
        return false;
    return /^[a-zA-Z0-9][a-zA-Z0-9_.:-]{4,138}[a-zA-Z0-9]$/.test(raw);
}
function tritVectorFromInput(args) {
    if (Array.isArray(args.trit_vector))
        return args.trit_vector.map((x) => normalizeTrit(x));
...
function normalizeLibraryRow(row) {
    ...
    return {
        ...
        signature_tokens: Array.isArray(src.signature_tokens)
            ? src.signature_tokens.map((x) => normalizeWordToken(x, 40)).filter(Boolean).slice(0, 64)
...
function trimLibrary(paths, policy) {
    ...
    fs.writeFileSync(paths.library_path, keep.map((row) => JSON.stringify(row)).join('\n') + '\n', 'utf8');
    return keep;
}
...
function tritSimilarity(queryVector, entryTrit) {
    ...
    if (majority === trit)
        return 1;
    if (majority === 0 || trit === 0)
        return 0.6;
    return 0;
}
function computeLibraryMatchScore(query, row, policy) {
    ...
    const totalWeight = Math.max(0.0001, Number(w.token_weight || 0) + Number(w.trit_weight || 0) + Number(w.target_weight || 0));
    const score = ((tokenScore * Number(w.token_weight || 0))
        + (tritScore * Number(w.trit_weight || 0))
        + (targetScore * Number(w.target_weight || 0))) / totalWeight;
    return Number(clampNumber(score, 0, 1, 0).toFixed(6));
}
function selectLibraryCandidates(paths, policy, query) {
    const rows = readJsonl(paths.library_path).map(normalizeLibraryRow);
    ...
    const scored = rows
        ...
        if (b.candidate_certainty !== a.candidate_certainty)
            return b.candidate_certainty - a.candidate_certainty;
        return String(b.row.ts || '').localeCompare(String(a.row.ts || ''));
    });
    return scored;
}
function currentRuntimeMode(args, policy) {
    const envMode = normalizeMode(process.env.INVERSION_RUNTIME_MODE || '');
    if (process.env.INVERSION_RUNTIME_MODE)
        return envMode;
    if (args.mode != null)
        return normalizeMode(args.mode);
    return normalizeMode(policy.runtime && policy.runtime.mode || 'live');
}
function certaintyThreshold(policy, band, impact) {
...
function maxTargetRankForDecision(policy, maturityBand, impact) {
...
function parseLaneDecision(args, paths, dateStr) {
    const lane = normalizeToken(args.brain_lane || args['brain-lane'] || args.generation_lane || args['generation-lane'], 120);
    if (lane)
        return { selected_lane: lane, source: 'arg', route: null };
    ...
    try {
        ...
        const selected = normalizeToken(routeObj.selected_lane
            || routeObj.lane
            || routeObj.brain
            || '', 120);
        return {
...
function evaluateCreativePenalty(policy, selectedLane) {
    const pref = policy.creative_preference || {};
    const preferred = Array.isArray(pref.preferred_creative_lane_ids)
        ? pref.preferred_creative_lane_ids.map((x) => normalizeToken(x, 120)).filter(Boolean)
        : [];
...
function loadActiveSessions(paths) {
...
function saveActiveSessions(paths, store) {
    const out = {
        schema_id: 'inversion_active_sessions',
        schema_version: '1.0',
        updated_at: nowIso(),
...
function emitEvent(paths, policy, dateStr, eventType, payload) {
...
function sweepExpiredSessions(paths, policy, dateStr) {
    ...
    if (expired.length === 0)
        return { expired_count: 0, sessions: store.sessions };
...
function computeKnownFailurePressure(candidates, policy) {
    const blockSimilarity = Number(policy.library.failed_repetition_similarity_block || 0.72);
    const failRows = candidates.filter((c) => c.row && c.row.outcome_trit === TRIT_PAIN);
    const hardBlock = failRows.some((row) => Number(row.similarity || 0) >= blockSimilarity);
    const similarityMax = failRows.reduce((acc, row) => Math.max(acc, Number(row.similarity || 0)), 0);
    return {
...
function evaluateRunDecision(args, policy, paths, maturityInfo, dateStr) {
    ...
    if (!tritVector.length)
        tritVector.push(trit);
    ...
    const externalSignalsCount = clampInt(args.external_signals_count || args['external-signals-count'], 0, 100000, 0);
    const evidenceCount = clampInt(args.evidence_count || args['evidence-count'], 0, 100000, 0);
    ...
    const effectiveCertainty = Number(clampNumber(certaintyInput - Number(creativePenalty.penalty || 0), 0, 1, 0).toFixed(6));
    ...
    const objectiveIdRequiredRank = clampInt(policy.guardrails && policy.guardrails.objective_id_required_min_target_rank, 1, 10, 2);
    ...
    const tierTransition = policy.tier_transition && typeof policy.tier_transition === 'object'
        ...
        : {};
    const transitionWindowDays = effectiveWindowDaysForTarget(tierTransition.window_days_by_target || {}, tierTransition.minimum_window_days_by_target || {}, target, 90);
    const useSuccessCountsForFirstN = toBool(tierTransition.use_success_counts_for_first_n, true);
    const safeAbortRelief = toBool(tierTransition.safe_abort_relief, true);
    const liveApplyAttemptCount = countTierEvents(tierScope, 'live_apply_attempts', target, transitionWindowDays);
    const liveApplySuccessCount = countTierEvents(tierScope, 'live_apply_successes', target, transitionWindowDays);
    const liveApplySafeAbortCount = countTierEvents(tierScope, 'live_apply_safe_aborts', target, transitionWindowDays);
    ...
    const firstNHumanVetoEnabled = (tierTransition.enabled === true
        && mode === 'live'
        && apply === true
    ...
    const shadowGate = policy.shadow_pass_gate && typeof policy.shadow_pass_gate === 'object'
        ...
        : {};
    const shadowWindowDays = windowDaysForTarget(shadowGate.window_days_by_target || {}, target, 90);
    const shadowPassCount = countTierEvents(tierScope, 'shadow_passes', target, shadowWindowDays);
    const shadowCriticalFailures = countTierEvents(tierScope, 'shadow_critical_failures', target, shadowWindowDays);
    const shadowGateActive = (shadowGate.enabled === true
        && shadowGate.require_for_live_apply === true
        && mode === 'live'
        && apply === true);
    const shadowPassRequired = shadowGateActive
        ? clampInt(shadowGate.required_passes_by_target && shadowGate.required_passes_by_target[target], 0, 100000, 0)
        : 0;
    const shadowCriticalMax = shadowGateActive
        ? clampInt(shadowGate.max_critical_failures_by_target && shadowGate.max_critical_failures_by_target[target], 0, 100000, 0)
        : 0;
    ...
    const successCandidate = libraryCandidates.find((entry) => entry.row.outcome_trit !== TRIT_PAIN);
    const reasons = [];
    const checks = {
    ...
    if (objectiveIdRequired && !objectiveId)
        reasons.push('objective_id_required_for_target_tier');
    if (objectiveId && !objectiveIdValid)
        reasons.push('objective_id_invalid_for_target_tier');
    if (targetRank > maxTargetRank)
        reasons.push('target_rank_exceeds_maturity_or_impact_gate');
    if (mode === 'live' && targetPolicy.live_enabled !== true)
        reasons.push('target_disabled_live');
    if (mode === 'test' && targetPolicy.test_enabled !== true)
        reasons.push('target_disabled_test');
    if (mode === 'test' && target === 'constitution') {
        if (policy.runtime.test.allow_constitution_inversion !== true)
            reasons.push('constitution_test_disabled_by_policy');
        if (allowConstitutionTest !== true)
            reasons.push('constitution_test_flag_required');
    }
    ...
    if (immutableAxiomHits.length > 0)
        reasons.push('immutable_axiom_violation');
    if (failurePressure.hard_block === true)
        reasons.push('known_failed_filter_stack_block');
    if (failurePressure.fail_count > maxSimilarFailures)
        reasons.push('similar_failures_above_band_limit');
    let certaintyFromLibrary = null;
    let reusedLibraryEntry = null;
    ...
    if (targetPolicy.require_human_veto_live === true && mode === 'live' && apply === true) {
        if (!approverId || !approvalNote)
            reasons.push('human_veto_required_for_target');
    }
...
function createSession(paths, policy, decision, args) {
    ...
    const ts = nowIso();
    const ttlMin = clampInt(args.session_ttl_min || args['session-ttl-min'], 5, 7 * 24 * 60, Number(policy.guardrails.default_session_ttl_minutes || 180));
...
function appendLibraryEntry(paths, policy, row) {
    appendJsonl(paths.library_path, {
        ...
        signature_tokens: Array.isArray(row.signature_tokens) && row.signature_tokens.length
            ? row.signature_tokens.map((x) => normalizeWordToken(x, 40)).filter(Boolean).slice(0, 64)
...
function recordTest(paths, policy, args, source) {
    ...
    const normalizedResult = source === 'record-test'
        ? (() => {
            if (rawToken === 'pass' || rawToken === 'success')
                return 'pass';
            if (rawToken === 'destructive')
                return 'destructive';
            return 'fail';
        })()
    ...
    if (normalizedResult === 'pass')
        stats.passed_tests = Math.max(0, Number(stats.passed_tests || 0)) + 1;
    else
        stats.failed_tests = Math.max(0, Number(stats.failed_tests || 0)) + 1;
    if (normalizedResult === 'destructive')
        stats.destructive_failures = Math.max(0, Number(stats.destructive_failures || 0)) + 1;
    if (safe)
        stats.safe_failures = Math.max(0, Number(stats.safe_failures || 0)) + 1;
    state.stats = stats;
...
function runNodeJson(scriptPath, argv, timeoutMs) {
...
function runHarnessRuntimeProbes(policy, dateStr) {
    ...
    if (cfg.run_red_team === true) {
        const redTeam = runNodeJson(path.join(ROOT, 'systems', 'autonomy', 'red_team_harness.js'), [
            'run',
            dateStr,
        ...
        if (!executionPass)
            out.reasons.push('runtime_probe_red_team_execution_missing');
        if (executionPass && !pass)
            out.reasons.push('runtime_probe_red_team_failed');
    }
    if (cfg.run_workflow_nursery === true) {
        const nurseryRun = runNodeJson(path.join(ROOT, 'systems', 'workflow', 'orchestron', 'adaptive_controller.js'), [
            'run',
            dateStr,
        ...
        if (nurseryRun.payload && nurseryRun.payload.output_path) {
            ...
            if (full && typeof full === 'object') {
                ...
                maxRegressionRisk = scorecards.reduce((acc, row) => Math.max(acc, Number(row && row.regression_risk || 0)), 0);
            }
        }
        ...
        const executionPass = (nurseryRun.code === 0
        ...
        if (!executionPass)
            out.reasons.push('runtime_probe_workflow_nursery_execution_missing');
        if (executionPass && !pass)
            out.reasons.push('runtime_probe_workflow_nursery_failed');
    }
    out.pass = out.reasons.length === 0;
    return out;
}
function runMaturityHarnessCycle(paths, policy, dateStr, opts = {}) {
    ...
    for (let i = 0; i < Math.min(maxTests, suite.length); i += 1) {
        ...
        const destructiveHit = destructiveTokens.some((token) => token && haystack.includes(token));
...
function maybeAutoRunHarness(paths, policy, dateStr, args) {
    const cfg = policy.maturity_harness && typeof policy.maturity_harness === 'object'
        ? policy.maturity_harness
        : {};
    if (cfg.enabled !== true) {
...
function extractFirstPrinciple(paths, policy, session, args, result) {
    if (policy.first_principles && policy.first_principles.enabled !== true)
        return null;
    if (result !== 'success')
        return null;
    ...
    const text = principleText || (autoExtract
        ? cleanText(`For ${cleanText(session.objective || 'objective', 180)}, use inversion filters (${(session.filter_stack || []).join(', ') || 'none'}) with a guarded ${normalizeTarget(session.target || 'tactical')} lane, then revert to baseline paradigm.`, 360)
        : '');
    if (!text)
        return null;
    const confidence = Number(clampNumber(Number(session.certainty || 0) * 0.7 + (session.fallback_entry_id ? 0.15 : 0.05), 0, 1, 0.5).toFixed(6));
    const principle = {
        ...
        strategy_feedback: {
            enabled: true,
            suggested_bonus: Number(clampNumber(confidence * Number(policy.first_principles.max_strategy_bonus || 0.12), 0, Number(policy.first_principles.max_strategy_bonus || 0.12), 0).toFixed(6))
        },
        session_id: cleanText(session.session_id || '', 80)
    };
    return principle;
}
function extractFailureClusterPrinciple(paths, policy, session) {
    if (policy.first_principles && policy.first_principles.enabled !== true)
        return null;
    if (policy.first_principles.allow_failure_cluster_extraction !== true)
        return null;
    const query = {
    ...
    const candidates = selectLibraryCandidates(paths, policy, query)
        .filter((entry) => entry.row && entry.row.outcome_trit === TRIT_PAIN);
    const clusterMin = Number(policy.first_principles.failure_cluster_min || 4);
    if (candidates.length < clusterMin)
        return null;
    const avgSimilarity = candidates.reduce((acc, row) => acc + Number(row.similarity || 0), 0) / Math.max(1, candidates.length);
    const confidence = Number(clampNumber((Math.min(1, candidates.length / (clusterMin + 3)) * 0.6) + (avgSimilarity * 0.4), 0, 1, 0.5).toFixed(6));
    const principle = {
        ...
        objective: cleanText(session.objective || '', 240),
        objective_id: cleanText(session.objective_id || '', 140) || null,
        statement: cleanText(`Avoid repeating inversion filter stack (${(session.filter_stack || []).join(', ') || 'none'}) for objective "${session.objective || 'unknown'}" without introducing a materially different paradigm shift.`, 360),
        target: normalizeTarget(session.target || 'tactical'),
        confidence,
        polarity: -1,
...
function persistFirstPrinciple(paths, session, principle) {
...
function cmdRun(args) {
    ...
    const interfaceEnvelope = buildOutputInterfaces(policy, out.mode, {
        ts: out.ts,
...
function cmdResolve(args) {
    ...
    const idx = sessions.findIndex((row) => String(row.session_id || '') === sessionId);
    if (idx < 0) {
        process.stdout.write(`${JSON.stringify({
            ok: false,
            type: 'inversion_resolve',
    ...
    let principleBlockReason = null;
    if (principle) {
        const downgradeCheck = checkFirstPrincipleDowngrade(paths, policy, session, Number(clampNumber(principle.confidence, 0, 1, 0)));
    ...
    const interfaces = buildOutputInterfaces(policy, receipt.mode, {
        ts: receipt.ts,
        session_id: sessionId,
...
function main() {
    ...
    if (cmd === 'record-test' || cmd === 'record_test')
        return cmdRecordTest(args);
    if (cmd === 'harness')
