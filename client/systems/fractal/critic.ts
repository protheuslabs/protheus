...
function proposeDomains(snapshot: any) {
  ...
  domains.push({
    id: 'habit_code',
    target_path: 'client/systems/adaptive/habits/habit_runtime_sync.ts',
    risk_tier: 2,
  ...
  domains.push({
    id: 'memory_schema',
    target_path: 'client/systems/ops/schema_evolution_contract.ts',
    risk_tier: 2,
  ...
  domains.push({
    id: 'routing_policy',
    target_path: 'client/systems/routing/model_router.ts',
    risk_tier: 2,
  ...
  if (Number.isFinite(drift) && drift >= 0.03) {
    domains.push({
      id: 'drift_control',
      target_path: 'client/systems/autonomy/drift_target_governor.ts',
      risk_tier: 2,
  ...
  if (pressure === 'high' || pressure === 'critical') {
    domains.push({
      id: 'burn_guard',
      target_path: 'client/systems/ops/dynamic_burn_budget_oracle.ts',
      risk_tier: 2,
  ...
  if (authority !== 'stream_authority' || !mirrored) {
    domains.push({
      ...
      target_path: 'client/systems/ops/event_sourced_control_plane.ts',
      risk_tier: 1,
