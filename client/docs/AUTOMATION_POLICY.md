...
When `node client/systems/spine/spine.js eyes|daily` runs, these stages run deterministically:
...
- If `AUTONOMY_ENABLED=1`: `client/systems/autonomy/autonomy_controller.js run` executes in daily spine.
...
- External actuation execution (`client/systems/actuation/actuation_executor.js run ...`)
- Model catalog apply operations (`model_catalog_loop.js apply ...`) with approval note
- Any break-glass guarded operation under `client/systems/security/guard.js`
...
- Check current health: `node client/systems/autonomy/health_status.js`
- View collector reliability: `node client/habits/scripts/external_eyes.js doctor`
- Force one collector probe run: `node client/habits/scripts/external_eyes.js canary`
- Inspect queue: `node client/habits/scripts/sensory_queue.js list --date=YYYY-MM-DD`
...
3. `client/systems/budget/system_budget.js status` projection pressure
