...
`client/systems/actuation/universal_execution_primitive.js` is the profile-driven execution entrypoint for capability profiles.

## Why

- Routes actions through one governed primitive path over capability profiles.
- Avoids bespoke per-tool execution branches.
- Preserves existing safety gates by delegating to `client/systems/actuation/actuation_executor.js`.

## Commands

```bash
# Execute from profile id (loaded from configured profile roots)
node client/systems/actuation/universal_execution_primitive.js run \
...
node client/systems/actuation/universal_execution_primitive.js status
```
...
Policy file: `client/config/universal_execution_primitive_policy.json`
