...
`client/systems/autonomy/gated_self_improvement_loop.ts` provides a governed self-improvement controller with staged rollout and automatic rollback.
...
```bash
node client/systems/autonomy/gated_self_improvement_loop.js propose --objective-id=hardening --target-path=client/systems/autonomy/gated_self_improvement_loop.ts
node client/systems/autonomy/gated_self_improvement_loop.js run --proposal-id=<id> --apply=0
node client/systems/autonomy/gated_self_improvement_loop.js status
node client/systems/autonomy/gated_self_improvement_loop.js rollback --proposal-id=<id> --reason=manual_revert
```
