...
Purpose: prevent drift while `client/systems/` + `client/lib/` are in phased TypeScript migration.
...
- `client/systems/security/repo_hygiene_guard.js` now fails strict mode when a changed file in `client/systems/` or `client/lib/` has a TS/JS twin and only one side changed.
