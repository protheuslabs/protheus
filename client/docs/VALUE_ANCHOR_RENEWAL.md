...
`client/systems/echo/value_anchor_renewal.ts` recalibrates long-horizon value anchors from constitution + first-principles evidence and emits reversible, auditable updates.
...
Policy file: `client/config/value_anchor_renewal_policy.json`
...
node client/systems/echo/value_anchor_renewal.js run --apply=0

# Apply renewal with explicit review metadata when required
node client/systems/echo/value_anchor_renewal.js run --apply=1 --approved-by=operator --approval-note="reviewed"

# Inspect status
node client/systems/echo/value_anchor_renewal.js status
```
