...
- Default path: `client/config/secret_broker_policy.json`
...
node client/systems/security/secret_broker.js resolve --handle=<token> --scope=<scope>
...
node client/systems/security/secret_rotation_migration_auditor.js attest --operator-id=$USER --approval-note=\"rotation_complete\" --apply=1
```
...
- `SPINE_SECRET_BROKER_POLICY_PATH=/abs/or/relative/path.json` (default `client/config/secret_broker_policy.json`)
