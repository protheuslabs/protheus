...
`node client/systems/autonomy/health_status.js [YYYY-MM-DD]`
2. If unsafe behavior is live, engage kill switch first:
`node client/systems/security/emergency_stop.js engage --scope=all --approval-note="contain incident"`
...
`node client/systems/sensory/adaptive_layer_guard.js run --strict`
...
1. `node client/systems/autonomy/health_status.js [YYYY-MM-DD]`
2. `node client/systems/routing/model_router.js doctor --risk=low --complexity=low --intent=ops_triage --task="routing incident diagnosis"`
3. `node client/systems/routing/model_router.js probe-all`
4. `node client/systems/routing/model_router.js stats`

Containment:

- Optional scope-limited stop while recovering:
`node client/systems/security/emergency_stop.js engage --scope=routing --approval-note="routing containment"`

Recovery:

1. Unban false positives if needed:
`node client/systems/routing/model_router.js unban --model=ollama/<name>`
2. Force safe execution mode if routing quality is uncertain:
`node client/systems/autonomy/strategy_mode.js set --mode=score_only --approval-note="routing degraded; reduce blast radius"`
...
1. `node client/systems/security/schema_contract_check.js run`
2. `node client/systems/spine/contract_check.js`
3. `node client/systems/sensory/adaptive_layer_guard.js run --strict`

Containment:

`node client/systems/security/emergency_stop.js engage --scope=autonomy,routing,actuation --approval-note="schema drift containment"`
...
1. `node client/habits/scripts/external_eyes.js doctor`
2. `node client/habits/scripts/external_eyes.js slo [YYYY-MM-DD]`
3. `node client/systems/spine/spine.js eyes [YYYY-MM-DD] --max-eyes=3`
4. `node client/systems/autonomy/proposal_enricher.js run [YYYY-MM-DD] --dry-run`

Containment:

- Keep autonomy in safer mode while sensory is degraded:
`node client/systems/autonomy/strategy_mode.js set --mode=score_only --approval-note="sensory degraded; hold execution risk"`

Recovery:

1. Refresh focus triggers:
`node client/systems/sensory/focus_controller.js refresh [YYYY-MM-DD]`
...
1. `node client/systems/autonomy/health_status.js [YYYY-MM-DD]`
2. `node client/systems/autonomy/strategy_readiness.js run [YYYY-MM-DD] --days=14`
3. `node client/systems/autonomy/pipeline_spc_gate.js run [YYYY-MM-DD] --days=1 --baseline-days=7 --sigma=3`
4. `node client/systems/autonomy/receipt_summary.js run [YYYY-MM-DD] --days=7`
5. `node client/systems/autonomy/strategy_mode_governor.js status [YYYY-MM-DD] --days=14`

Containment:

- keep/return to score-only until pass rates recover:
`node client/systems/autonomy/strategy_mode.js set --mode=score_only --approval-note="autonomy stall containment"`

Recovery:

1. Recompute admission metadata:
`node client/systems/autonomy/proposal_enricher.js run [YYYY-MM-DD]`
2. Run governor in dry mode to inspect proposed transition:
`node client/systems/autonomy/strategy_mode_governor.js run [YYYY-MM-DD] --days=14 --dry-run`
...
1. `node client/habits/scripts/sensory_queue.js stats --days=7`
2. `node client/habits/scripts/sensory_queue.js list --status=open --days=7`
3. `node client/systems/autonomy/health_status.js [YYYY-MM-DD]` (check queue backlog SLO + recovery pulse)

Containment / Recovery:

1. Run deterministic hygiene:
`node client/habits/scripts/queue_gc.js run [YYYY-MM-DD]`
`node client/habits/scripts/sensory_queue.js sweep [YYYY-MM-DD]`
2. Force compact terminal churn:
`node client/systems/ops/queue_log_compact.js run --apply=1`
3. If backlog is budget-constrained, pin pressure explicitly for a run:
`QUEUE_GC_BUDGET_PRESSURE=hard node client/habits/scripts/queue_gc.js run [YYYY-MM-DD]`
...
1. `node client/systems/autonomy/health_status.js [YYYY-MM-DD]`
2. `node client/systems/memory/idle_dream_cycle.js status`
3. `node client/systems/memory/idle_dream_cycle.js run [YYYY-MM-DD] --force=1`

Containment / Recovery:

1. If repeated cloud/local timeout loops occur, keep dream lane degraded until stable:
`IDLE_DREAM_FORCE_DEGRADED=1 node client/systems/memory/idle_dream_cycle.js run [YYYY-MM-DD] --force=1`
2. Reduce dream pressure during instability:
`IDLE_DREAM_MAX_ITEMS=6 node client/systems/memory/idle_dream_cycle.js run [YYYY-MM-DD] --force=1`
...
1. `node client/systems/autonomy/health_status.js [YYYY-MM-DD]`
2. `node client/systems/budget/system_budget.js status`
3. `node client/systems/budget/system_budget.js tail --limit=50`

Containment / Recovery:

1. Keep autonomy in score-only while pressure is unresolved:
`node client/systems/autonomy/strategy_mode.js set --mode=score_only --approval-note="budget pressure containment"`
...
1. `node client/systems/autonomy/health_status.js [YYYY-MM-DD]`
2. `node client/systems/autonomy/receipt_summary.js run [YYYY-MM-DD] --days=7`
3. `node client/systems/autonomy/slo_runbook_check.js run [YYYY-MM-DD]`
...
`node client/systems/autonomy/strategy_mode.js set --mode=score_only --approval-note="verification pass-rate containment"`
...
`node client/systems/security/emergency_stop.js engage --scope=autonomy --approval-note="rollback drill"`
3. Run core guards:
`node client/systems/security/schema_contract_check.js run`
`node client/systems/sensory/adaptive_layer_guard.js run --strict`
4. Release stop:
`node client/systems/security/emergency_stop.js release --approval-note="rollback drill complete"`
...
1. `node client/systems/ops/offsite_backup.js status`
2. `node client/systems/ops/offsite_backup.js list --limit=3`
...
2. Run strict sync:
`node client/systems/ops/offsite_backup.js sync --strict=1`
3. Run strict restore drill:
`node client/systems/ops/offsite_backup.js restore-drill --strict=1`
...
1. `node client/systems/security/secure_heartbeat_endpoint.js verify --strict=1`
2. `node client/systems/security/secure_heartbeat_endpoint.js status`
...
`node client/systems/security/secure_heartbeat_endpoint.js revoke-key --key-id=<id> --reason="incident_response"`
2. Rotate client key:
`node client/systems/security/secure_heartbeat_endpoint.js issue-key --client-id=<client>`
...
`cat client/config/cron_jobs.json`
...
3. Inspect policy:
`cat client/config/rust_source_of_truth_policy.json`
...
1. `routing_degraded`
   - Run: `node client/systems/routing/model_router.js doctor --risk=low --complexity=low --intent=ops_triage --task="routing incident diagnosis"`
2. `schema_drift`
   - Run: `node client/systems/security/schema_contract_check.js run --strict=1`
3. `sensory_starvation`
   - Run: `node client/habits/scripts/external_eyes.js preflight --strict=0`
4. `autonomy_stall`
   - Run: `node client/systems/autonomy/ops_dashboard.js status`
...
2. Execute rollback target check:
   - `node client/systems/autonomy/improvement_controller.js evaluate --force=1 --auto-revert=1`
