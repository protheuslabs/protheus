...
- `node client/systems/ops/dist_runtime_cutover.js status`
- `node client/systems/ops/dist_runtime_cutover.js set-mode --mode=dist`
- `node client/systems/ops/dist_runtime_cutover.js set-mode --mode=source`
- `node client/systems/ops/dist_runtime_cutover.js verify --build=1 --strict=1`
...
- `client/systems/spine/contract_check.js` now fails if runtime mode resolves to `dist` while `PROTHEUS_RUNTIME_DIST_REQUIRED` is not `1`.
- Optional strict wrapper coverage check:
  - `CONTRACT_CHECK_DIST_WRAPPER_STRICT=1 node client/systems/spine/contract_check.js`
