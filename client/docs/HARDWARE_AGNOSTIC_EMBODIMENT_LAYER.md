...
`client/systems/hardware/embodiment_layer.ts` provides a unified capability + surface-budget contract so core runtime lanes can stay hardware-neutral.
...
Policy file: `client/config/embodiment_layer_policy.json`
...
node client/systems/hardware/embodiment_layer.js sense --profile=auto

# Verify profile parity invariants (ignoring configured capacity fields)
node client/systems/hardware/embodiment_layer.js verify-parity --profiles=phone,desktop,cluster --strict=1

# Read latest snapshot
node client/systems/hardware/embodiment_layer.js status
```
