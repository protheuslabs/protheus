...
Acceptance: every enabled job in `client/config/cron_jobs.json` has `delivery.mode=announce` and `delivery.channel=main`.
...
Acceptance: `client/config/model_health_recovery_policy.json` and `client/config/model_health_auto_recovery_policy.json` define bounded backoff and fallback model bindings for `llama3.2:*`.
...
Acceptance: scheduled secret-rotation attestation job exists in `client/config/cron_jobs.json` and emits deterministic remediation output when stale.
...
Policy file:
- `client/config/f100_enterprise_hardening_policy.json`
...
- `cargo run --quiet --manifest-path core/layer0/ops/Cargo.toml --bin protheus-ops -- sdlc-change-control run --strict=1 --policy=client/config/sdlc_change_control_policy.json --pr-body-path=state/ops/sdlc_change_control/pr_body.md --changed-paths-path=state/ops/sdlc_change_control/changed_paths.txt`
