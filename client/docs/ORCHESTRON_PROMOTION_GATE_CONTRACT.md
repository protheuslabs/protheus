...
- `client/systems/workflow/workflow_controller.ts` (`promote` command)
- `client/config/workflow_policy.json` (`promotion_gate`)
...
```bash
node client/systems/workflow/workflow_controller.js promote \
  --source=promotable \
  --status=active \
  --ignore-threshold=1 \
  --approver-id=<id> \
  --approval-note="..." \
  --policy=client/config/workflow_policy.json
```
