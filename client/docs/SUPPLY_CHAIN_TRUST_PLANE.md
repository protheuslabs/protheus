...
`client/systems/security/supply_chain_trust_plane.ts` provides deterministic build trust artifacts and release-gate verification.
...
Policy file: `client/config/supply_chain_trust_policy.json`
...
node client/systems/security/supply_chain_trust_plane.js run --strict=1

# Verification-only lane used by merge guard
node client/systems/security/supply_chain_trust_plane.js run --strict=1 --verify-only=1

# Inspect latest status
node client/systems/security/supply_chain_trust_plane.js status
```

## Release gate

`client/systems/security/merge_guard.ts` now runs the supply-chain trust plane in strict verification mode.
