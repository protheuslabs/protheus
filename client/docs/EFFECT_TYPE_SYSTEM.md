...
`client/systems/primitives/effect_type_system.ts` enforces plan-time effect governance for workflow execution.
...
Policy file: `client/config/effect_type_policy.json`
...
`client/systems/workflow/workflow_executor.ts` evaluates effect plans before any step execution.
...
```bash
node client/systems/primitives/effect_type_system.js evaluate --workflow-json=@/tmp/workflow.json --strict=1
node client/systems/primitives/effect_type_system.js status
```
