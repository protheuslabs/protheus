...
`client/systems/ops/backlog_registry.ts` is the canonical backlog governance lane.
...
- `client/systems/ops/backlog_queue_executor.ts` is a receipt/materialization lane.
...
- `client/systems/ops/backlog_lane_batch_delivery.ts` provides lane execution coverage for IDs that lacked dedicated lane scripts.
- Policy: `client/config/backlog_lane_batch_delivery_policy.json`
