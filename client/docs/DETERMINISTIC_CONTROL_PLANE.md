...
`client/systems/distributed/deterministic_control_plane.js` provides a deterministic control-plane baseline for sovereign multi-node operation.

## Policy

- `client/config/deterministic_control_plane_policy.json`
...
```bash
node client/systems/distributed/deterministic_control_plane.js run --nodes-json='[...]' --apply=1
node client/systems/distributed/deterministic_control_plane.js status
```
