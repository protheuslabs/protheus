...
Date: 2026-02-26  
Scope: `client/systems/eye/eye_kernel.ts`
...
```bash
node client/systems/eye/eye_kernel.js route --lane=<lane> --target=<target> --action=<action> [--risk=low|medium|high|critical] [--clearance=L0|L1|L2|L3] [--estimated-tokens=N] [--apply=1|0]
node client/systems/eye/eye_kernel.js status [--date=YYYY-MM-DD]
```

## Policy Contract

`client/config/eye_kernel_policy.json` defines:
- `clearance_levels[]`
