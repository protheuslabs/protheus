#!/usr/bin/env node
/**
 * client/habits/scripts/queue_gc.js — deterministic queue backpressure + auto-triage
 *
 * Goal:
 ...
 *
 * Usage:
 *   node client/habits/scripts/queue_gc.js run [YYYY-MM-DD] [--cap-per-eye=N] [--ttl-hours=N]
 *
 * Env:
...
function loadBudgetPressure(repo, dateStr) {
  ...
  try {
    const status = spawnSync(
      "node",
      ["client/systems/budget/system_budget.js", "status", dateStr, "--request_tokens_est=0"],
...
function main() {
  ...
  if (!mode || mode === "--help" || mode === "-h") {
    console.log("Usage:");
    console.log("  node client/habits/scripts/queue_gc.js run [YYYY-MM-DD] [--cap-per-eye=N] [--cap-per-type=N] [--ttl-hours=N]");
    process.exit(0);
  }

  if (mode !== "run") {
    console.error("Usage:");
    console.error("  node client/habits/scripts/queue_gc.js run [YYYY-MM-DD] [--cap-per-eye=N] [--cap-per-type=N] [--ttl-hours=N]");
    process.exit(2);
  }
