 ...
 * 
 * Commands:
 *   node client/habits/scripts/external_eyes.js run [--eye=<id>] [--max-eyes=N]
 *   node client/habits/scripts/external_eyes.js preflight [--strict]
 *   node client/habits/scripts/external_eyes.js canary-signal [--eye=<id>]
 *   node client/habits/scripts/external_eyes.js score [YYYY-MM-DD]
 *   node client/habits/scripts/external_eyes.js evolve [YYYY-MM-DD]
 *   node client/habits/scripts/external_eyes.js list
 *   node client/habits/scripts/external_eyes.js propose "<name>" "<domain>" "<notes>"
 */
...
function buildEyesIntakeCommand(payload) {
  ...
  const chunks = [
    'node client/systems/sensory/eyes_intake.js create',
...
function buildInfrastructureOutageProposal(dateStr, outageInfo) {
  const nextCommand = 'node client/systems/routing/route_execute.js --task="Diagnose shared transport outage affecting multiple non-stub eyes; verify DNS/network/auth path and apply minimal deterministic fix." --tokens_est=1200 --repeats_14d=2 --errors_30d=1 --dry-run';
  const validation = [
  ...
  if (out.triggered && EYES_SLO_EMIT_PAIN) {
    try {
      emitPainSignal({
        ...
        suggested_next_command: 'node client/habits/scripts/external_eyes.js doctor'
      });
    } catch {
      // best-effort only
    }
...
function buildCollectorRemediationProposal(eyeConfig, registryEye, threshold) {
  ...
  const nextCommand = `node client/systems/routing/route_execute.js --task="Diagnose repeated collector fetch failures for eye ${eyeId}; implement minimal deterministic fix; preserve sensing-only behavior." --tokens_est=900 --repeats_14d=2 --errors_30d=1 --dry-run`;
  const validation = [
...
function buildCollectorEscalationProposal(eyeConfig, registryEye, failureCode, failureHits, threshold) {
  ...
  const nextCommand = `node client/systems/routing/route_execute.js --task="Investigate persistent self-heal failure for eye ${eyeId} (code=${failureCode}); diagnose root cause and propose/implement bounded deterministic fix with rollback." --tokens_est=1200 --repeats_14d=3 --errors_30d=1 --dry-run`;
  const validation = [
  ...
  if (configMutated) {
    try {
      setCatalog(CONFIG_PATH, config, {
        source: 'client/habits/scripts/external_eyes.js',
