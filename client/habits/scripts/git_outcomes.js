#!/usr/bin/env node
/**
 * client/habits/scripts/git_outcomes.js — deterministic git→outcome bridge
 *
 * Purpose:
 ...
 *
 * Commands:
 *   node client/habits/scripts/git_outcomes.js run [YYYY-MM-DD]
 *
 * State:
...
function assertProposalQueueInterface() {
  const rel = "client/habits/scripts/proposal_queue.js";
  if (!existsRel(rel)) {
...
function usage() {
  console.log("Usage:");
  console.log("  node client/habits/scripts/git_outcomes.js run [YYYY-MM-DD]");
}
...
function main() {
  ...
  for (const { id, sha } of uniquePairs) {
    ...
    const res = runNode([
      "client/habits/scripts/proposal_queue.js",
      "outcome",
      id,
      "shipped",
      evidence
