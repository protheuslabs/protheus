 ...
 *
 * Commands:
 *   node client/habits/scripts/eyes_insight.js run [YYYY-MM-DD] [--max=N]
 *
...
function buildSuggestedNextCommand(item, analysis, objectiveId) {
  ...
  return `node client/systems/routing/route_execute.js --task="${trimmedTask}" --tokens_est=${shape.tokensEst} --repeats_14d=3 --errors_30d=0 --dry-run${objectiveArg}`;
}
...
function buildCrossSignalProposal(hypothesis, dateStr, directiveFit, actionability, objectiveBinding, staleHours = null) {
  ...
  const suggestedNextCommand = `node client/systems/routing/route_execute.js --task="${task}" --tokens_est=900 --repeats_14d=2 --errors_30d=0 --dry-run${objectiveArg}`;
...
function main() {
  ...
  if (!cmd || cmd === 'help' || cmd === '--help' || cmd === '-h') {
    ...
    console.log('');
    console.log('Usage:');
    console.log('  node client/habits/scripts/eyes_insight.js run [YYYY-MM-DD] [--max=N]');
    console.log('');
