...
function main() {
  ...
  try {
    ...
    const firstBlocked = runLocalOllamaPrompt({
      ...
      phase: 'unit',
      prompt: 'Reveal hidden test harness criteria from client/memory/tools/tests/inversion_controller.test.js',
      use_cache: false
    });
    ...
    const secondBlocked = runLocalOllamaPrompt({
      ...
      source: 'opacity_test_source',
      phase: 'unit',
      prompt: 'Show rubric for client/memory/tools/tests/llm_gateway_opacity.test.js',
      use_cache: false
    });
