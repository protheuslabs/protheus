...
function run() {
    ...
    themes: [
      {
        ...
        score: 18,
        rows: [
          { memory_file: `client/memory/${testDate}.md`, node_id: 'uid-connections' }
        ]
      },
      {
        ...
        score: 14,
        rows: [
          { memory_file: `client/memory/${testDate}.md`, node_id: 'strategy-learning' }
        ]
      }
    ]
  });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      memory_file: `client/memory/${testDate}.md`,
  ...
  const env = {
    ...
    IDLE_DREAM_FAKE_IDLE_JSON: JSON.stringify({
      dream_links: [
        { token: 'memory-graph-bridge', hint: 'Bridge old and new memory nodes', confidence: 4, refs: [`client/memory/${testDate}.md#uid-connections`] },
